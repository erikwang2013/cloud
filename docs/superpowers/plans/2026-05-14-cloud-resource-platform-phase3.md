# Phase 3: 运营完善 — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task.

**Goal:** 工单系统、消息通知、监控告警、管理后台页面、客户端 App 骨架、部署配置。平台达到可正式运营状态。

**Architecture:** 工单/通知通过事件驱动 + Redis Queue 异步分发。管理后台基于 webman-admin 搭建。客户端 App 共享 API 层。

**Tech Stack:** PHP 8.2+, webman-admin, webman redis-queue, Flutter 3.x, HarmonyOS ArkTS, Docker

---

### Task 3.1: Ticket system

**Files:**
- Create: `service/app/ticket/controller/TicketController.php`
- Create: `service/app/ticket/service/TicketService.php`
- Create: `service/app/ticket/model/Ticket.php`
- Create: `service/app/ticket/model/TicketMessage.php`
- Create: `service/app/ticket/event/TicketCreated.php`
- Create: `service/app/ticket/listener/AutoAssignListener.php`
...
- [ ] **Step 3: Create AutoAssignListener**
...
**Files:**
- Create: `service/app/notification/service/NotificationDispatcher.php`
- Create: `service/app/notification/queue/EmailSender.php`
- Create: `service/app/notification/queue/SmsSender.php`
- Create: `service/app/notification/queue/PushSender.php`
- Create: `service/app/notification/model/Notification.php`
- Create: `service/app/notification/model/NotificationTemplate.php`

- [ ] **Step 1: Create NotificationDispatcher**
...
- [ ] **Step 2: Create EmailSender queue worker**
...
- [ ] **Step 3: Create SmsSender queue worker**
...
- [ ] **Step 4: Create PushSender queue worker**
...
**Files:**
- Create: `service/app/monitor/controller/MonitorController.php`
- Create: `service/app/monitor/service/ResourceMonitor.php`
- Create: `service/app/monitor/service/AlertEngine.php`
- Create: `service/app/monitor/cron/CollectMetrics.php`
- Create: `service/app/monitor/cron/CheckExpirations.php`

- [ ] **Step 1: Create ResourceMonitor**
...
- [ ] **Step 4: Create admin monitoring dashboard controller**
...
### Task 3.4: Admin backend (webman-admin pages)

**Files:**
- Create: `service/app/admin/controller/DashboardController.php`
- Create: `service/app/admin/controller/UserController.php`
- Create: `service/app/admin/controller/ProductController.php`
- Create: `service/app/admin/controller/OrderController.php`
- Create: `service/app/admin/controller/PaymentController.php`
- Create: `service/app/admin/controller/SupplierController.php`
...
- [ ] **Step 2: Create Order management admin controllers**
...
### Task 3.5: Flutter app skeleton

**Files:**
- Create: `apps/flutter/` project structure
- Create: `apps/flutter/lib/core/` (network, auth, i18n, theme)
- Create: `apps/flutter/lib/features/` (auth, products, orders, resources)
...
- [ ] **Step 2: Create core network layer** — `apps/flutter/lib/core/network/api_client.dart`
...
### Task 3.6: HarmonyOS app skeleton

**Files:**
- Create: `apps/harmonyos/` project structure

- [ ] **Step 1: Create HarmonyOS project structure**
...
### Task 3.7: Docker and deployment configuration
...
### Task 3.8: Reporting service

**Files:**
- Create: `service/app/report/service/ReportService.php`
- Create: `service/app/report/controller/ReportController.php`
...
### Task 3.9: Final integration — wire all routes, queues, and events

- [ ] **Step 1: Complete route config** — `service/config/route.php`
...
- [ ] **Step 2: Register queue consumers in config**
...
- [ ] **Step 3: Register event listeners in start.php**
...
**Phase 3 complete. Platform is fully operational:**
- Users: register → browse → order → pay → auto-provision → manage resources → submit tickets
- Admin: dashboard → manage users/products/orders/payments → review KYC → handle tickets → view reports
- Supplier: apply → get approved → list products → receive settlements → withdraw
- Monitoring: 5-min metrics collection, alerts on failures/expirations
