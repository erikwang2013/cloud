# Phase 3 : Perfectionnement de l'exploitation — Plan d'implémentation

> **Pour les agents autonomes :** SOUS-COMPÉTENCE OBLIGATOIRE : utilisez superpowers:subagent-driven-development (recommandé) ou superpowers:executing-plans pour implémenter ce plan tâche par tâche.

**Objectif :** système de tickets, notifications de messages, alertes de surveillance, pages du panneau d'administration, squelette des applications clientes, configuration de déploiement. La plateforme atteint un état opérationnel complet.

**Architecture :** les tickets/notifications sont distribués de manière asynchrone via des événements + file Redis. Le panneau d'administration repose sur webman-admin. Les applications clientes partagent la couche API.

**Pile technique :** PHP 8.2+, webman-admin, webman redis-queue, Flutter 3.x, HarmonyOS ArkTS, Docker

---

### Tâche 3.1 : Système de tickets

**Fichiers :**
- Créer : `service/app/ticket/controller/TicketController.php`
- Créer : `service/app/ticket/service/TicketService.php`
- Créer : `service/app/ticket/model/Ticket.php`
- Créer : `service/app/ticket/model/TicketMessage.php`
- Créer : `service/app/ticket/event/TicketCreated.php`
- Créer : `service/app/ticket/listener/AutoAssignListener.php`

- [ ] **Étape 1 : Créer TicketService**
...
- [ ] **Étape 2 : Créer TicketController**
...
- [ ] **Étape 3 : Créer AutoAssignListener**
...
### Tâche 3.2 : Système de notifications

**Fichiers :**
- Créer : `service/app/notification/service/NotificationDispatcher.php`
- Créer : `service/app/notification/queue/EmailSender.php`
- Créer : `service/app/notification/queue/SmsSender.php`
- Créer : `service/app/notification/queue/PushSender.php`
- Créer : `service/app/notification/model/Notification.php`
- Créer : `service/app/notification/model/NotificationTemplate.php`

- [ ] **Étape 1 : Créer NotificationDispatcher**
...
- [ ] **Étape 2 : Créer le worker EmailSender (file)**
...
- [ ] **Étape 3 : Créer le worker SmsSender (file)**
...
- [ ] **Étape 4 : Créer le worker PushSender (file)**
...
### Tâche 3.3 : Surveillance et alertes

**Fichiers :**
- Créer : `service/app/monitor/controller/MonitorController.php`
- Créer : `service/app/monitor/service/ResourceMonitor.php`
- Créer : `service/app/monitor/service/AlertEngine.php`
- Créer : `service/app/monitor/cron/CollectMetrics.php`
- Créer : `service/app/monitor/cron/CheckExpirations.php`

- [ ] **Étape 1 : Créer ResourceMonitor**
...
- [ ] **Étape 4 : Créer le contrôleur du tableau de bord de surveillance admin**
...
### Tâche 3.4 : Panneau d'administration (pages webman-admin)

**Fichiers :**
- Créer : `service/app/admin/controller/DashboardController.php`
- Créer : `service/app/admin/controller/UserController.php`
- Créer : `service/app/admin/controller/ProductController.php`
- Créer : `service/app/admin/controller/OrderController.php`
- Créer : `service/app/admin/controller/PaymentController.php`
- Créer : `service/app/admin/controller/SupplierController.php`
...
- [ ] **Étape 2 : Créer les contrôleurs admin de gestion des commandes**
...
### Tâche 3.5 : Squelette d'application Flutter

**Fichiers :**
- Créer : structure du projet `apps/flutter/`
- Créer : `apps/flutter/lib/core/` (network, auth, i18n, theme)
- Créer : `apps/flutter/lib/features/` (auth, products, orders, resources)

- [ ] **Étape 1 : Initialiser le projet Flutter**

```bash
cd apps && flutter create --org com.cloudplatform --project-name cloud_platform flutter
```

- [ ] **Étape 2 : Créer la couche réseau core** — `apps/flutter/lib/core/network/api_client.dart`
...
- [ ] **Étape 3 : Créer la structure des fonctionnalités**
...
### Tâche 3.6 : Squelette d'application HarmonyOS

**Fichiers :**
- Créer : structure du projet `apps/harmonyos/`

- [ ] **Étape 1 : Créer la structure du projet HarmonyOS**
...
### Tâche 3.7 : Configuration Docker et de déploiement

**Fichiers :**
- Créer : `docker/Dockerfile`
- Créer : `docker/docker-compose.yml`
- Créer : `docker/nginx.conf`
- Créer : `docker/supervisor.conf`
- Créer : `service/.env.example`
...
### Tâche 3.8 : Service de reporting

**Fichiers :**
- Créer : `service/app/report/service/ReportService.php`
- Créer : `service/app/report/controller/ReportController.php`

- [ ] **Étape 1 : Créer ReportService**
...
### Tâche 3.9 : Intégration finale — câbler toutes les routes, files et événements

- [ ] **Étape 1 : Compléter la configuration des routes** — `service/config/route.php`
...
- [ ] **Étape 2 : Enregistrer les consommateurs de files dans la configuration**
...
- [ ] **Étape 3 : Enregistrer les écouteurs d'événements dans start.php**
...
**Phase 3 terminée. La plateforme est pleinement opérationnelle :**
- Utilisateurs : inscription → navigation → commande → paiement → provisionnement automatique → gestion des ressources → dépôt de tickets
- Administration : tableau de bord → gestion des utilisateurs/produits/commandes/paiements → examen des KYC → traitement des tickets → consultation des rapports
- Fournisseurs : candidature → approbation → mise en ligne des produits → réception des règlements → retrait
- Surveillance : collecte des métriques toutes les 5 minutes, alertes en cas de pannes/expirations
- Déploiement : démarrage en une commande avec Docker Compose
