# Phase 0 : Infrastructure — Plan d'implémentation

> **Pour les agents de travail :** SOUS-COMPÉTENCE REQUISE : utilisez superpowers:subagent-driven-development (recommandé) ou superpowers:executing-plans pour implémenter ce plan tâche par tâche. Les étapes utilisent la syntaxe des cases à cocher (`- [ ]`) pour le suivi.

**Objectif :** mettre en place l'échafaudage du projet, les migrations de base de données, la bibliothèque commune, l'authentification et l'autorisation, l'infrastructure multilingue et les middlewares de sécurité, afin de fournir la base de toutes les phases suivantes.

**Architecture :** webman monolithe modulaire, découpé en modules de répertoires indépendants par domaine métier, l'infrastructure commune étant placée dans common/. Base de données MySQL + ORM Eloquent, cache/file d'attente Redis.

**Pile technologique :** PHP 8.2+, webman 1.6+, illuminate/database, firebase/php-jwt, redis, monolog

---

## Structure des fichiers (cette phase)
...
### Tâche 0.1 : Initialiser le projet webman et les dépendances

**Fichiers :**
- Créer : `service/composer.json`
...
Exécuter : `cd service && composer install`
Résultat attendu : tous les packages installés, vendor/ créé.

- [ ] **Étape 3 : Créer config/app.php**
...
- [ ] **Étape 4 : Créer config/database.php**
...
- [ ] **Étape 5 : Créer config/redis.php**
...
- [ ] **Étape 7 : Créer config/security.php**
...
- [ ] **Étape 9 : Créer le bootstrap start.php**
...
### Tâche 0.2 : Créer toutes les migrations de base de données

**Fichiers :**
- Créer : `service/database/migrations/0001_create_users_tables.php`

- [ ] **Étape 1 : Créer la migration users**
...
- [ ] **Étape 2 : Créer la migration products** — `service/database/migrations/0002_create_product_tables.php`
...
- [ ] **Étape 3 : Créer la migration orders** — `service/database/migrations/0003_create_order_tables.php`
...
- [ ] **Étape 4 : Migrations 0004-0010**

Continuer avec les migrations restantes en suivant le même modèle :
...
Chacune suit le même modèle que ci-dessus avec `Capsule::schema()->create(...)`.

- [ ] **Étape 5 : Créer l'exécuteur de migrations** — `service/database/migrate.php`
...
- [ ] **Étape 6 : Exécuter toutes les migrations**

Exécuter : `php service/database/migrate.php`
Résultat attendu : toutes les tables créées avec succès.
...
### Tâche 0.3 : Construire la bibliothèque d'aides communes

**Fichiers :**
- Créer : `service/common/helper/Response.php`
- Créer : `service/common/helper/Validator.php`

- [ ] **Étape 1 : Créer l'aide Response**
...
- [ ] **Étape 2 : Créer l'aide request_id**
...
### Tâche 0.4 : Construire l'authentification JWT

**Fichiers :**
- Créer : `service/common/auth/JwtAuth.php`
- Créer : `service/common/auth/middleware/AuthMiddleware.php`

- [ ] **Étape 1 : Créer JwtAuth**
...
### Tâche 0.5 : Construire le RBAC

**Fichiers :**
- Créer : `service/common/auth/Rbac.php`
- Créer : `service/common/auth/middleware/RbacMiddleware.php`
...
### Tâche 0.6 : Construire l'infrastructure i18n

**Fichiers :**
- Créer : `service/common/i18n/I18n.php`
- Créer : `service/common/i18n/middleware/LocaleMiddleware.php`
- Créer : `service/i18n/zh-CN/messages.php`
- Créer : `service/i18n/en-US/messages.php`
...
- [ ] **Étape 2 : Créer LocaleMiddleware**
...
- [ ] **Étape 3 : Créer les fichiers de messages i18n**
...
### Tâche 0.7 : Construire les middlewares de sécurité

**Fichiers :**
- Créer : `service/common/security/WafMiddleware.php`
- Créer : `service/common/security/RateLimitMiddleware.php`
- Créer : `service/common/security/CorsMiddleware.php`
- Créer : `service/common/security/AuditLogger.php`
- Créer : `service/common/security/LogSanitizer.php`

- [ ] **Étape 1 : Créer WafMiddleware**
...
- [ ] **Étape 2 : Créer RateLimitMiddleware**
...
### Tâche 0.8 : Créer l'endpoint de contrôle d'état et enregistrer le pipeline de middlewares

**Fichiers :**
- Créer : `service/app/controller/HealthController.php`
- Modifier : `service/start.php` (enregistrement des middlewares)
- Modifier : `service/config/route.php` (s'il existe, sinon créer)
...
- [ ] **Étape 2 : Enregistrer les middlewares globaux dans start.php**
...
- [ ] **Étape 3 : Créer la configuration des routes**
...
### Tâche 0.9 : Vérifier l'exhaustivité de la phase 0

- [ ] **Étape 1 : Démarrer le serveur de développement webman**

Exécuter : `cd service && php start.php start`
Résultat attendu : le serveur démarre sur http://0.0.0.0:8787

- [ ] **Étape 2 : Tester l'endpoint de contrôle d'état**

Exécuter : `curl http://localhost:8787/health`
Résultat attendu : `{"code":0,"message":"ok","data":{"status":"healthy",...}}`

- [ ] **Étape 3 : Tester la limitation de débit**

Exécuter : `for i in {1..10}; do curl -s http://localhost:8787/health; done`
Résultat attendu : toutes les requêtes renvoient 200 (health n'a pas de limitation stricte)

- [ ] **Étape 4 : Tester le WAF**

Exécuter : `curl -X POST http://localhost:8787/api/auth/login -d "email=test' OR '1'='1"`
Résultat attendu : `{"code":403,"message":"Request blocked by WAF"}`
...
- [ ] **Étape 6 : Committer le fichier de verrouillage**
...
**Phase 0 terminée.** Base prête : échafaudage du projet, 50+ tables de base de données, auth JWT, RBAC, i18n, WAF + limitation de débit + CORS, journalisation d'audit.
