...
- [ ] `GET /api/v1/products` → 200 (valid JSON)
- [ ] `POST /api/v1/auth/login` (no body) → 422 (parameter validation)
- [ ] `GET /api/v1/user/profile` (no token) → 401 (authentication)
- [ ] URL version: `curl /api/v99/products` → 400 (unsupported version)
