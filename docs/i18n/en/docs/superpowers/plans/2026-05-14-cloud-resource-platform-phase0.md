...
**Goal:** Build the project scaffold, database migrations, common library, authentication and authorization, multi-language infrastructure, and security middleware, providing the foundation for all later phases.

**Architecture:** webman modular monolith, split into independent directory modules by business domain, with common infrastructure under common/. The database uses MySQL + Eloquent ORM, and Redis is used for cache/queue.
