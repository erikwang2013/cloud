...
DELETE /api/v1/user/account (GDPR deletion)
...
POST /api/v1/products/{id}/reviews (login required)
  → rating (1-5) + content
...
GET /api/v1/cart           → Cart list (with SKU details + live prices)
...
2. POST /api/v1/coupons/validate                  Apply coupon
     → {code: "SAVE10"}
     ← {coupon_id, discount, type: percent/fixed}

3. GET /api/v1/orders/{id}/payment-methods        Get available payment channels
     → PaymentRouter::getAvailableChannels(order)
     ← [{channel_id, name, code, amount, fee, total_amount}]

4. POST /api/v1/orders/{id}/pay                   Initiate payment
...
```
POST /api/v1/supplier/apply (user login required)
...
```
GET /api/v1/domain/check/{domain}/{tld}    # Domain availability
GET /api/v1/domain/tlds                     # Registrable TLD list (cached 1h)
GET /api/v1/dns/{domain}                    # DNS record list
POST /api/v1/dns/{domain}/records           # Add DNS record
DELETE /api/v1/dns/{domain}/records/{id}    # Delete DNS record (password confirmation)
```
...
Admin:
  GET /admin/api/v1/tickets              # Ticket queue
  POST /admin/api/v1/tickets/{id}/assign # Assign support agent
  POST /admin/api/v1/tickets/{id}/close  # Close ticket
...
Admin API:
  GET /admin/api/v1/features → list all Flags with status/source
...
**Cache purge:** `POST /api/v1/cdn/domains/{id}/purge`, URLs are automatically deduplicated and trimmed (max 100), only the domain itself or its subdomains are allowed, wildcards and external URLs are rejected, idempotent.
...
Provides two endpoints: POST /graphql (public queries) and POST /api/v1/graphql (authenticated queries). Built on webonyx/graphql-php, query depth limited to 5 levels, complexity limited to 100.
