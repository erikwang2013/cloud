# Fase 0: Infraestrutura — Plano de Implementação

> **Para agentes de trabalho:** SUB-HABILIDADE OBRIGATÓRIA: use superpowers:subagent-driven-development (recomendado) ou superpowers:executing-plans para implementar este plano tarefa por tarefa. As etapas usam a sintaxe de caixa de seleção (`- [ ]`) para acompanhamento.

**Objetivo:** montar o scaffolding do projeto, migrações de banco de dados, biblioteca comum, autenticação e autorização, infraestrutura multilíngue e middlewares de segurança, fornecendo a base para todas as fases seguintes.

**Arquitetura:** monólito modular em webman, dividido em diretórios-módulo independentes por domínio de negócio; a infraestrutura comum fica em common/. Banco de dados MySQL + Eloquent ORM; cache/fila via Redis.

**Stack Tecnológica:** PHP 8.2+, webman 1.6+, illuminate/database, firebase/php-jwt, redis, monolog

---

## Estrutura de Arquivos (esta fase)
...
### Tarefa 0.1: Inicializar o projeto webman e as dependências

**Arquivos:**
- Criar: `service/composer.json`

- [ ] **Passo 1: Criar o composer.json**
...
Execute: `cd service && composer install`
Esperado: todos os pacotes instalados, vendor/ criado.

- [ ] **Passo 3: Criar o config/app.php**
...
- [ ] **Passo 4: Criar o config/database.php**
...
- [ ] **Passo 5: Criar o config/redis.php**
...
- [ ] **Passo 6: Criar o config/auth.php**
...
- [ ] **Passo 7: Criar o config/security.php**
...
- [ ] **Passo 8: Criar o config/i18n.php**
...
- [ ] **Passo 9: Criar o bootstrap start.php**
...
### Tarefa 0.2: Criar todas as migrações de banco de dados

**Arquivos:**
- Criar: `service/database/migrations/0001_create_users_tables.php`

- [ ] **Passo 1: Criar a migração de usuários**
...
- [ ] **Passo 2: Criar a migração de produtos** — `service/database/migrations/0002_create_product_tables.php`
...
- [ ] **Passo 3: Criar a migração de pedidos** — `service/database/migrations/0003_create_order_tables.php`
...
- [ ] **Passo 4: Migrações 0004-0010**

Continue com as migrações restantes seguindo o mesmo padrão:
...
Cada uma segue o mesmo padrão acima com `Capsule::schema()->create(...)`.

- [ ] **Passo 5: Criar o executor de migrações** — `service/database/migrate.php`
...
- [ ] **Passo 6: Executar todas as migrações**

Execute: `php service/database/migrate.php`
Esperado: todas as tabelas criadas com sucesso.
...
### Tarefa 0.3: Construir a biblioteca de helpers comum

**Arquivos:**
- Criar: `service/common/helper/Response.php`
- Criar: `service/common/helper/Validator.php`

- [ ] **Passo 1: Criar o helper Response**
...
- [ ] **Passo 2: Criar o helper request_id**
...
### Tarefa 0.4: Construir a autenticação JWT

**Arquivos:**
- Criar: `service/common/auth/JwtAuth.php`
- Criar: `service/common/auth/middleware/AuthMiddleware.php`

- [ ] **Passo 1: Criar o JwtAuth**
...
- [ ] **Passo 2: Criar o AuthMiddleware**
...
### Tarefa 0.5: Construir o RBAC

**Arquivos:**
- Criar: `service/common/auth/Rbac.php`
- Criar: `service/common/auth/middleware/RbacMiddleware.php`

- [ ] **Passo 1: Criar o Rbac**
...
- [ ] **Passo 2: Criar o RbacMiddleware**
...
### Tarefa 0.6: Construir a infraestrutura de i18n

**Arquivos:**
- Criar: `service/common/i18n/I18n.php`
- Criar: `service/common/i18n/middleware/LocaleMiddleware.php`
- Criar: `service/i18n/zh-CN/messages.php`
- Criar: `service/i18n/en-US/messages.php`

- [ ] **Passo 1: Criar o I18n**
...
- [ ] **Passo 2: Criar o LocaleMiddleware**
...
- [ ] **Passo 3: Criar os arquivos de mensagens i18n**
...
### Tarefa 0.7: Construir os middlewares de segurança

**Arquivos:**
- Criar: `service/common/security/WafMiddleware.php`
- Criar: `service/common/security/RateLimitMiddleware.php`
- Criar: `service/common/security/CorsMiddleware.php`
- Criar: `service/common/security/AuditLogger.php`
- Criar: `service/common/security/LogSanitizer.php`

- [ ] **Passo 1: Criar o WafMiddleware**
...
- [ ] **Passo 2: Criar o RateLimitMiddleware**
...
- [ ] **Passo 3: Criar o CorsMiddleware**
...
### Tarefa 0.8: Criar o endpoint de saúde e registrar o pipeline de middlewares

**Arquivos:**
- Criar: `service/app/controller/HealthController.php`
- Modificar: `service/start.php` (registro de middlewares)
- Modificar: `service/config/route.php` (se existir; caso contrário, criar)

- [ ] **Passo 1: Criar o HealthController**
...
- [ ] **Passo 2: Registrar os middlewares globais no start.php**
...
- [ ] **Passo 3: Criar a configuração de rotas**
...
### Tarefa 0.9: Verificar a completude da Fase 0

- [ ] **Passo 1: Iniciar o servidor de desenvolvimento webman**

Execute: `cd service && php start.php start`
Esperado: o servidor inicia em http://0.0.0.0:8787

- [ ] **Passo 2: Testar o endpoint de saúde**

Execute: `curl http://localhost:8787/health`
Esperado: `{"code":0,"message":"ok","data":{"status":"healthy",...}}`

- [ ] **Passo 3: Testar a limitação de frequência**

Execute: `for i in {1..10}; do curl -s http://localhost:8787/health; done`
Esperado: todas retornam 200 (o endpoint de saúde não tem limite estrito de frequência)

- [ ] **Passo 4: Testar o WAF**

Execute: `curl -X POST http://localhost:8787/api/auth/login -d "email=test' OR '1'='1"` 
Esperado: `{"code":403,"message":"Request blocked by WAF"}`

- [ ] **Passo 5: Verificar a conexão com o banco**

Execute: `php service/database/migrate.php`
Esperado: todas as migrações executam sem erros.

- [ ] **Passo 6: Fazer commit do arquivo de lock**
...
**Fase 0 concluída.** Base pronta: scaffolding do projeto, 50+ tabelas no banco, autenticação JWT, RBAC, i18n, WAF + limitação de frequência + CORS, logs de auditoria.
