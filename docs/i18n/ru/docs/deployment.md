...
- [ ] `GET /api/v1/products` → 200 (корректный JSON)
- [ ] `POST /api/v1/auth/login` (без body) → 422 (проверка параметров)
- [ ] `GET /api/v1/user/profile` (без токена) → 401 (аутентификация)
- [ ] Проверка версии: `curl /api/v99/products` → 400
