# Fase 3: Perfeccionamiento operativo — Plan de implementación

> **Para agentes autónomos:** SUB-HABILIDAD OBLIGATORIA: use superpowers:subagent-driven-development (recomendado) o superpowers:executing-plans para implementar este plan tarea por tarea.

**Objetivo:** sistema de tickets, notificaciones de mensajes, monitoreo de alertas, páginas del panel de administración, esqueleto de la app cliente, configuración de despliegue. La plataforma alcanza un estado listo para operar formalmente.

**Arquitectura:** los tickets/notificaciones se distribuyen de forma asíncrona mediante eventos + Redis Queue. El panel de administración se construye sobre webman-admin. Las apps cliente comparten la capa de API.

**Stack tecnológico:** PHP 8.2+, webman-admin, webman redis-queue, Flutter 3.x, HarmonyOS ArkTS, Docker

---

### Tarea 3.1: Sistema de tickets

**Archivos:**
- Crear: `service/app/ticket/controller/TicketController.php`
- Crear: `service/app/ticket/service/TicketService.php`
- Crear: `service/app/ticket/model/Ticket.php`
- Crear: `service/app/ticket/model/TicketMessage.php`
- Crear: `service/app/ticket/event/TicketCreated.php`
- Crear: `service/app/ticket/listener/AutoAssignListener.php`

- [ ] **Paso 1: Crear TicketService**
...
- [ ] **Paso 3: Crear AutoAssignListener**
...
### Tarea 3.2: Sistema de notificaciones

**Archivos:**
- Crear: `service/app/notification/service/NotificationDispatcher.php`
- Crear: `service/app/notification/queue/EmailSender.php`
- Crear: `service/app/notification/queue/SmsSender.php`
- Crear: `service/app/notification/queue/PushSender.php`
- Crear: `service/app/notification/model/Notification.php`
- Crear: `service/app/notification/model/NotificationTemplate.php`

- [ ] **Paso 1: Crear NotificationDispatcher**
...
- [ ] **Paso 2: Crear el worker de cola EmailSender**
...
- [ ] **Paso 3: Crear el worker de cola SmsSender**
...
- [ ] **Paso 4: Crear el worker de cola PushSender**
...
### Tarea 3.3: Monitoreo y alertas

**Archivos:**
- Crear: `service/app/monitor/controller/MonitorController.php`
- Crear: `service/app/monitor/service/ResourceMonitor.php`
- Crear: `service/app/monitor/service/AlertEngine.php`
- Crear: `service/app/monitor/cron/CollectMetrics.php`
- Crear: `service/app/monitor/cron/CheckExpirations.php`

- [ ] **Paso 1: Crear ResourceMonitor**
...
- [ ] **Paso 4: Crear el controlador del panel de monitoreo de administración**
...
### Tarea 3.4: Panel de administración (páginas webman-admin)

**Archivos:**
- Crear: `service/app/admin/controller/DashboardController.php`
- Crear: `service/app/admin/controller/UserController.php`
- Crear: `service/app/admin/controller/ProductController.php`
- Crear: `service/app/admin/controller/OrderController.php`
- Crear: `service/app/admin/controller/PaymentController.php`
- Crear: `service/app/admin/controller/SupplierController.php`
...
- [ ] **Paso 2: Crear los controladores de administración de pedidos**
...
### Tarea 3.5: Esqueleto de la app Flutter

**Archivos:**
- Crear: estructura del proyecto `apps/flutter/`
- Crear: `apps/flutter/lib/core/` (red, auth, i18n, tema)
- Crear: `apps/flutter/lib/features/` (auth, productos, pedidos, recursos)

- [ ] **Paso 1: Inicializar el proyecto Flutter**

```bash
cd apps && flutter create --org com.cloudplatform --project-name cloud_platform flutter
```

- [ ] **Paso 2: Crear la capa de red central** — `apps/flutter/lib/core/network/api_client.dart`
...
- [ ] **Paso 3: Crear la estructura de funciones**
...
### Tarea 3.6: Esqueleto de la app HarmonyOS

**Archivos:**
- Crear: estructura del proyecto `apps/harmonyos/`

- [ ] **Paso 1: Crear la estructura del proyecto HarmonyOS**
...
### Tarea 3.8: Servicio de informes

**Archivos:**
- Crear: `service/app/report/service/ReportService.php`
- Crear: `service/app/report/controller/ReportController.php`

- [ ] **Paso 1: Crear ReportService**
...
### Tarea 3.9: Integración final — cablear todas las rutas, colas y eventos

- [ ] **Paso 1: Completar la configuración de rutas** — `service/config/route.php`
...
- [ ] **Paso 2: Registrar los consumidores de cola en la configuración**
...
- [ ] **Paso 3: Registrar los listeners de eventos en start.php**
...
**Fase 3 completa. La plataforma está totalmente operativa:**
- Usuarios: registrarse → explorar → pedir → pagar → aprovisionamiento automático → gestionar recursos → enviar tickets
- Administración: panel → gestionar usuarios/productos/pedidos/pagos → revisar KYC → atender tickets → ver informes
- Proveedores: solicitar → ser aprobado → publicar productos → recibir liquidaciones → retirar fondos
- Monitoreo: recopilación de métricas cada 5 minutos, alertas ante fallos/caducidades
- Despliegue: puesta en marcha con un solo comando mediante Docker Compose
