# Fase 0: Infraestructura — Plan de implementación

> **Para trabajadores agénticos:** SUB-SKILL REQUERIDO: usa superpowers:subagent-driven-development (recomendado) o superpowers:executing-plans para implementar este plan tarea por tarea. Los pasos usan sintaxis de casilla de verificación (`- [ ]`) para el seguimiento.

**Objetivo:** montar el andamiaje del proyecto, las migraciones de base de datos, la biblioteca común, la autenticación/autorización, la infraestructura multilingüe y los middleware de seguridad, proporcionando la base para todas las fases posteriores.

**Arquitectura:** monolito modular webman, dividido en directorios de módulos independientes por dominio de negocio; la infraestructura común se coloca en common/. La base de datos usa MySQL + Eloquent ORM; la caché/colas usan Redis.

**Pila tecnológica:** PHP 8.2+, webman 1.6+, illuminate/database, firebase/php-jwt, redis, monolog

---

## Estructura de archivos (esta fase)
...
### Tarea 0.1: Inicializar el proyecto webman y las dependencias
...
Resultado esperado: todos los paquetes instalados, se crea vendor/.

- [ ] **Paso 3: Crear config/app.php**
...
- [ ] **Paso 4: Crear config/database.php**
...
- [ ] **Paso 7: Crear config/security.php**
...
- [ ] **Paso 9: Crear el arranque start.php**
...
### Tarea 0.2: Crear todas las migraciones de base de datos

**Archivos:**
- Crear: `service/database/migrations/0001_create_users_tables.php`

- [ ] **Paso 1: Crear la migración de usuarios**
...
- [ ] **Paso 2: Crear la migración de productos** — `service/database/migrations/0002_create_product_tables.php`
...
- [ ] **Paso 3: Crear la migración de pedidos** — `service/database/migrations/0003_create_order_tables.php`
...
- [ ] **Paso 4: Migraciones 0004-0010**

Continuar con las migraciones restantes siguiendo el mismo patrón:
...
Cada una sigue el mismo patrón que las anteriores con `Capsule::schema()->create(...)`.

- [ ] **Paso 5: Crear el ejecutor de migraciones** — `service/database/migrate.php`
...
- [ ] **Paso 6: Ejecutar todas las migraciones**

Ejecutar: `php service/database/migrate.php`
Resultado esperado: todas las tablas creadas correctamente.
...
### Tarea 0.3: Crear la biblioteca común de helpers

**Archivos:**
- Crear: `service/common/helper/Response.php`
- Crear: `service/common/helper/Validator.php`

- [ ] **Paso 1: Crear el helper Response**
...
- [ ] **Paso 2: Crear el helper request_id**
...
### Tarea 0.4: Crear la autenticación JWT

**Archivos:**
- Crear: `service/common/auth/JwtAuth.php`
- Crear: `service/common/auth/middleware/AuthMiddleware.php`

- [ ] **Paso 1: Crear JwtAuth**
...
### Tarea 0.5: Crear RBAC

**Archivos:**
- Crear: `service/common/auth/Rbac.php`
- Crear: `service/common/auth/middleware/RbacMiddleware.php`
...
### Tarea 0.6: Crear la infraestructura i18n

**Archivos:**
- Crear: `service/common/i18n/I18n.php`
- Crear: `service/common/i18n/middleware/LocaleMiddleware.php`
- Crear: `service/i18n/zh-CN/messages.php`
- Crear: `service/i18n/en-US/messages.php`
...
- [ ] **Paso 3: Crear los archivos de mensajes i18n**
...
### Tarea 0.7: Crear los middleware de seguridad

**Archivos:**
- Crear: `service/common/security/WafMiddleware.php`
- Crear: `service/common/security/RateLimitMiddleware.php`
- Crear: `service/common/security/CorsMiddleware.php`
- Crear: `service/common/security/AuditLogger.php`
- Crear: `service/common/security/LogSanitizer.php`

- [ ] **Paso 1: Crear WafMiddleware**
...
- [ ] **Paso 2: Crear RateLimitMiddleware**
...
### Tarea 0.8: Crear el endpoint de salud y registrar la cadena de middleware

**Archivos:**
- Crear: `service/app/controller/HealthController.php`
- Modificar: `service/start.php` (registro de middleware)
- Modificar: `service/config/route.php` (si existe; si no, crearlo)
...
- [ ] **Paso 2: Registrar el middleware global en start.php**
...
- [ ] **Paso 3: Crear la configuración de rutas**
...
### Tarea 0.9: Verificar la integridad de la Fase 0

- [ ] **Paso 1: Arrancar el servidor de desarrollo webman**

Ejecutar: `cd service && php start.php start`
Resultado esperado: el servidor arranca en http://0.0.0.0:8787

- [ ] **Paso 2: Probar el endpoint de salud**

Ejecutar: `curl http://localhost:8787/health`
Resultado esperado: `{"code":0,"message":"ok","data":{"status":"healthy",...}}`

- [ ] **Paso 3: Probar el límite de velocidad**

Ejecutar: `for i in {1..10}; do curl -s http://localhost:8787/health; done`
Resultado esperado: todos devuelven 200 (health no tiene límite de velocidad estricto)

- [ ] **Paso 4: Probar el WAF**

Ejecutar: `curl -X POST http://localhost:8787/api/auth/login -d "email=test' OR '1'='1"` 
Resultado esperado: `{"code":403,"message":"Request blocked by WAF"}`

- [ ] **Paso 5: Verificar la conexión a la BD**

Ejecutar: `php service/database/migrate.php`
Resultado esperado: todas las migraciones se ejecutan sin errores.

- [ ] **Paso 6: Confirmar el archivo lock**
...
**Fase 0 completada.** Base lista: andamiaje del proyecto, más de 50 tablas de BD, autenticación JWT, RBAC, i18n, WAF + límite de velocidad + CORS, registro de auditoría.
