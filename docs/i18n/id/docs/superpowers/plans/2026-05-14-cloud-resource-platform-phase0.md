# Phase 0: Infrastruktur — Implementation Plan

> **Untuk agen agentic:** SUB-SKILL WAJIB: Gunakan superpowers:subagent-driven-development (direkomendasikan) atau superpowers:executing-plans untuk mengimplementasikan rencana ini tugas-per-tugas. Langkah menggunakan sintaks checkbox (`- [ ]`) untuk pelacakan.

**Tujuan:** Membangun scaffolding proyek, migrasi database, pustaka umum, autentikasi & otorisasi, infrastruktur multi-bahasa, middleware keamanan, menyediakan fondasi untuk semua fase berikutnya.

**Arsitektur:** webman modular monolith, dipecah menjadi modul direktori independen berdasarkan domain bisnis, infrastruktur publik ditempatkan di common/. Database menggunakan MySQL + Eloquent ORM, cache/antrean menggunakan Redis.

**Tech Stack:** PHP 8.2+, webman 1.6+, illuminate/database, firebase/php-jwt, redis, monolog

---

## Struktur File (fase ini)
...
### Tugas 0.1: Inisialisasi proyek webman dan dependensi

**File:**
- Buat: `service/composer.json`

- [ ] **Langkah 1: Buat composer.json**
...
Ekspektasi: Semua paket terinstal, vendor/ dibuat.

- [ ] **Langkah 3: Buat config/app.php**
...
- [ ] **Langkah 4: Buat config/database.php**
...
- [ ] **Langkah 5: Buat config/redis.php**
...
- [ ] **Langkah 6: Buat config/auth.php**
...
- [ ] **Langkah 7: Buat config/security.php**
...
- [ ] **Langkah 8: Buat config/i18n.php**
...
- [ ] **Langkah 9: Buat bootstrap start.php**
...
### Tugas 0.2: Buat semua migrasi database

**File:**
- Buat: `service/database/migrations/0001_create_users_tables.php`

- [ ] **Langkah 1: Buat migrasi users**
...
- [ ] **Langkah 2: Buat migrasi products** — `service/database/migrations/0002_create_product_tables.php`
...
- [ ] **Langkah 3: Buat migrasi orders** — `service/database/migrations/0003_create_order_tables.php`
...
- [ ] **Langkah 4: Migrasi 0004-0010**

Lanjutkan dengan migrasi yang tersisa mengikuti pola yang sama:
...
Masing-masing mengikuti pola yang sama seperti di atas dengan `Capsule::schema()->create(...)`.

- [ ] **Langkah 5: Buat migration runner** — `service/database/migrate.php`
...
- [ ] **Langkah 6: Jalankan semua migrasi**

Jalankan: `php service/database/migrate.php`
Ekspektasi: Semua tabel berhasil dibuat.
...
### Tugas 0.3: Bangun pustaka helper umum

**File:**
- Buat: `service/common/helper/Response.php`
- Buat: `service/common/helper/Validator.php`

- [ ] **Langkah 1: Buat helper Response**
...
- [ ] **Langkah 2: Buat helper request_id**
...
### Tugas 0.4: Bangun autentikasi JWT

**File:**
- Buat: `service/common/auth/JwtAuth.php`
- Buat: `service/common/auth/middleware/AuthMiddleware.php`

- [ ] **Langkah 1: Buat JwtAuth**
...
### Tugas 0.5: Bangun RBAC

**File:**
- Buat: `service/common/auth/Rbac.php`
- Buat: `service/common/auth/middleware/RbacMiddleware.php`
...
### Tugas 0.6: Bangun infrastruktur i18n

**File:**
- Buat: `service/common/i18n/I18n.php`
- Buat: `service/common/i18n/middleware/LocaleMiddleware.php`
- Buat: `service/i18n/zh-CN/messages.php`
- Buat: `service/i18n/en-US/messages.php`
...
- [ ] **Langkah 2: Buat LocaleMiddleware**
...
- [ ] **Langkah 3: Buat file pesan i18n**
...
### Tugas 0.7: Bangun middleware keamanan

**File:**
- Buat: `service/common/security/WafMiddleware.php`
- Buat: `service/common/security/RateLimitMiddleware.php`
- Buat: `service/common/security/CorsMiddleware.php`
- Buat: `service/common/security/AuditLogger.php`
- Buat: `service/common/security/LogSanitizer.php`

- [ ] **Langkah 1: Buat WafMiddleware**
...
- [ ] **Langkah 2: Buat RateLimitMiddleware**
...
### Tugas 0.8: Buat endpoint health dan daftarkan pipeline middleware

**File:**
- Buat: `service/app/controller/HealthController.php`
- Ubah: `service/start.php` (registrasi middleware)
- Ubah: `service/config/route.php` (jika ada, jika tidak buat)
...
- [ ] **Langkah 2: Daftarkan middleware global di start.php**
...
- [ ] **Langkah 3: Buat konfigurasi route**
...
### Tugas 0.9: Verifikasi kelengkapan Phase 0

- [ ] **Langkah 1: Mulai server dev webman**

Jalankan: `cd service && php start.php start`
Ekspektasi: Server mulai di http://0.0.0.0:8787

- [ ] **Langkah 2: Uji endpoint health**

Jalankan: `curl http://localhost:8787/health`
Ekspektasi: `{"code":0,"message":"ok","data":{"status":"healthy",...}}`

- [ ] **Langkah 3: Uji rate limiting**

Jalankan: `for i in {1..10}; do curl -s http://localhost:8787/health; done`
Ekspektasi: Semua mengembalikan 200 (health tidak memiliki rate limit ketat)

- [ ] **Langkah 4: Uji WAF**

Jalankan: `curl -X POST http://localhost:8787/api/auth/login -d "email=test' OR '1'='1"` 
Ekspektasi: `{"code":403,"message":"Request blocked by WAF"}`

- [ ] **Langkah 5: Verifikasi koneksi DB**

Jalankan: `php service/database/migrate.php`
Ekspektasi: Semua migrasi berjalan tanpa error.

- [ ] **Langkah 6: Commit file lock**
...
**Phase 0 selesai.** Fondasi siap: scaffolding proyek, 50+ tabel DB, autentikasi JWT, RBAC, i18n, WAF + rate limiting + CORS, logging audit.
