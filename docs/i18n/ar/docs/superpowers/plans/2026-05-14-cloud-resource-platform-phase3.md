# المرحلة 3: إكمال العمليات — خطة التنفيذ

> **للوكلاء العاملين:** المهارة الفرعية المطلوبة: استخدم superpowers:subagent-driven-development (موصى بها) أو superpowers:executing-plans لتنفيذ هذه الخطة مهمةً بمهمة.

**الهدف:** نظام التذاكر، إشعارات الرسائل، مراقبة التنبيهات، صفحات لوحة الإدارة، هيكل تطبيق العميل، إعدادات النشر. تصل المنصة إلى حالة جاهزة للتشغيل الرسمي.

**البنية:** توزيع التذاكر/الإشعارات عبر معمارية الأحداث + Redis Queue بشكل غير متزامن. لوحة الإدارة مبنية على webman-admin. تطبيق العميل يشارك طبقة API.

**حزمة التقنيات:** PHP 8.2+، webman-admin، webman redis-queue، Flutter 3.x، HarmonyOS ArkTS، Docker

---

### المهمة 3.1: نظام التذاكر

**الملفات:**
- إنشاء: `service/app/ticket/controller/TicketController.php`
- إنشاء: `service/app/ticket/service/TicketService.php`
- إنشاء: `service/app/ticket/model/Ticket.php`
- إنشاء: `service/app/ticket/model/TicketMessage.php`
- إنشاء: `service/app/ticket/event/TicketCreated.php`
- إنشاء: `service/app/ticket/listener/AutoAssignListener.php`

- [ ] **الخطوة 1: إنشاء TicketService**
...
- [ ] **الخطوة 2: إنشاء TicketController**
...
- [ ] **الخطوة 3: إنشاء AutoAssignListener**
...
### المهمة 3.2: نظام الإشعارات

**الملفات:**
- إنشاء: `service/app/notification/service/NotificationDispatcher.php`
- إنشاء: `service/app/notification/queue/EmailSender.php`
- إنشاء: `service/app/notification/queue/SmsSender.php`
- إنشاء: `service/app/notification/queue/PushSender.php`
- إنشاء: `service/app/notification/model/Notification.php`
- إنشاء: `service/app/notification/model/NotificationTemplate.php`

- [ ] **الخطوة 1: إنشاء NotificationDispatcher**
...
- [ ] **الخطوة 2: إنشاء عامل قائمة انتظار EmailSender**
...
- [ ] **الخطوة 3: إنشاء عامل قائمة انتظار SmsSender**
...
- [ ] **الخطوة 4: إنشاء عامل قائمة انتظار PushSender**
...
### المهمة 3.3: المراقبة والتنبيهات

**الملفات:**
- إنشاء: `service/app/monitor/controller/MonitorController.php`
- إنشاء: `service/app/monitor/service/ResourceMonitor.php`
- إنشاء: `service/app/monitor/service/AlertEngine.php`
- إنشاء: `service/app/monitor/cron/CollectMetrics.php`
- إنشاء: `service/app/monitor/cron/CheckExpirations.php`

- [ ] **الخطوة 1: إنشاء ResourceMonitor**
...
- [ ] **الخطوة 4: إنشاء وحدة تحكم لوحة مراقبة الإدارة**
...
### المهمة 3.4: لوحة الإدارة (صفحات webman-admin)

**الملفات:**
- إنشاء: `service/app/admin/controller/DashboardController.php`
- إنشاء: `service/app/admin/controller/UserController.php`
- إنشاء: `service/app/admin/controller/ProductController.php`
- إنشاء: `service/app/admin/controller/OrderController.php`
- إنشاء: `service/app/admin/controller/PaymentController.php`
- إنشاء: `service/app/admin/controller/SupplierController.php`
...
- [ ] **الخطوة 2: إنشاء وحدات تحكم إدارة الطلبات**
...
### المهمة 3.5: هيكل تطبيق Flutter

**الملفات:**
- إنشاء: بنية مشروع `apps/flutter/`
- إنشاء: `apps/flutter/lib/core/` (الشبكة، المصادقة، i18n، السمة)
- إنشاء: `apps/flutter/lib/features/` (المصادقة، المنتجات، الطلبات، الموارد)

- [ ] **الخطوة 1: تهيئة مشروع Flutter**

```bash
cd apps && flutter create --org com.cloudplatform --project-name cloud_platform flutter
```

- [ ] **الخطوة 2: إنشاء طبقة الشبكة الأساسية** — `apps/flutter/lib/core/network/api_client.dart`
...
### المهمة 3.6: هيكل تطبيق HarmonyOS

**الملفات:**
- إنشاء: بنية مشروع `apps/harmonyos/`

- [ ] **الخطوة 1: إنشاء بنية مشروع HarmonyOS**
...
- [ ] **الخطوة 2: إنشاء ApiClient (ArkTS)**
...
### المهمة 3.7: إعدادات Docker والنشر

**الملفات:**
- إنشاء: `docker/Dockerfile`
- إنشاء: `docker/docker-compose.yml`
- إنشاء: `docker/nginx.conf`
- إنشاء: `docker/supervisor.conf`
- إنشاء: `service/.env.example`
...
### المهمة 3.8: خدمة التقارير

**الملفات:**
- إنشاء: `service/app/report/service/ReportService.php`
- إنشاء: `service/app/report/controller/ReportController.php`

- [ ] **الخطوة 1: إنشاء ReportService**
...
### المهمة 3.9: التكامل النهائي — ربط جميع المسارات وقوائم الانتظار والأحداث

- [ ] **الخطوة 1: إكمال إعداد المسارات** — `service/config/route.php`
...
- [ ] **الخطوة 2: تسجيل مستهلكي قائمة الانتظار في الإعداد**
...
- [ ] **الخطوة 3: تسجيل مستمعي الأحداث في start.php**
...
**اكتملت المرحلة 3. المنصة جاهزة للتشغيل الكامل:**
- المستخدمون: تسجيل ← تصفح ← طلب ← دفع ← فتح تلقائي ← إدارة الموارد ← تقديم التذاكر
- الإدارة: لوحة التحكم ← إدارة المستخدمين/المنتجات/الطلبات/المدفوعات ← مراجعة KYC ← معالجة التذاكر ← عرض التقارير
- الموردون: التقديم ← الموافقة ← إدراج المنتجات ← استلام التسويات ← السحب
- المراقبة: جمع المقاييس كل 5 دقائق، تنبيهات عند الفشل/الانتهاء
- النشر: تشغيل فوري بأمر واحد عبر Docker Compose
