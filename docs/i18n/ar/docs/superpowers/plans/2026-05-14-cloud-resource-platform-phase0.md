# المرحلة 0: البنية التحتية — خطة التنفيذ

> **للعاملين الوكلاء:** مهارة فرعية مطلوبة: استخدم superpowers:subagent-driven-development (موصى بها) أو superpowers:executing-plans لتنفيذ هذه الخطة مهمة بمهمة. تستخدم الخطوات صيغة مربع الاختيار (`- [ ]`) للتتبع.

**الهدف:** إعداد سقالات المشروع، وترحيلات قاعدة البيانات، والمكتبات العامة، والمصادقة والتفويض، وبنية تعدد اللغات، ووسائط الأمان، لتوفير الأساس لجميع المراحل اللاحقة.

**البنية:** webman متجانس معياري، مقسم إلى وحدات دلائل مستقلة حسب مجال الأعمال، مع وضع البنية التحتية العامة في common/. قاعدة البيانات MySQL + Eloquent ORM، والتخزين المؤقت/قوائم الانتظار Redis.

**المكدس التقني:** PHP 8.2+، webman 1.6+، illuminate/database، firebase/php-jwt، redis، monolog

---

## بنية الملفات (هذه المرحلة)
...
### المهمة 0.1: تهيئة مشروع webman والتبعيات
...
المتوقع: تثبيت جميع الحزم، وإنشاء vendor/.

- [ ] **الخطوة 3: إنشاء config/app.php**
...
- [ ] **الخطوة 4: إنشاء config/database.php**
...
- [ ] **الخطوة 5: إنشاء config/redis.php**
...
- [ ] **الخطوة 6: إنشاء config/auth.php**
...
- [ ] **الخطوة 7: إنشاء config/security.php**
...
- [ ] **الخطوة 8: إنشاء config/i18n.php**
...
- [ ] **الخطوة 9: إنشاء start.php (الإقلاع)**
...
### المهمة 0.2: إنشاء جميع ترحيلات قاعدة البيانات

**الملفات:**
- إنشاء: `service/database/migrations/0001_create_users_tables.php`

- [ ] **الخطوة 1: إنشاء ترحيل المستخدمين**
...
- [ ] **الخطوة 2: إنشاء ترحيل المنتجات** — `service/database/migrations/0002_create_product_tables.php`
...
- [ ] **الخطوة 3: إنشاء ترحيل الطلبات** — `service/database/migrations/0003_create_order_tables.php`
...
- [ ] **الخطوة 4: الترحيلات 0004-0010**

تابع مع بقية الترحيلات باتباع النمط نفسه:
...
يتبع كل منها النمط نفسه أعلاه مع `Capsule::schema()->create(...)`.

- [ ] **الخطوة 5: إنشاء مشغل الترحيل** — `service/database/migrate.php`
...
- [ ] **الخطوة 6: تشغيل جميع الترحيلات**

التشغيل: `php service/database/migrate.php`
المتوقع: إنشاء جميع الجداول بنجاح.
...
### المهمة 0.3: بناء مكتبة المساعدات العامة

**الملفات:**
- إنشاء: `service/common/helper/Response.php`
- إنشاء: `service/common/helper/Validator.php`

- [ ] **الخطوة 1: إنشاء مساعد Response**
...
- [ ] **الخطوة 2: إنشاء مساعد request_id**
...
### المهمة 0.4: بناء مصادقة JWT

**الملفات:**
- إنشاء: `service/common/auth/JwtAuth.php`
- إنشاء: `service/common/auth/middleware/AuthMiddleware.php`

- [ ] **الخطوة 1: إنشاء JwtAuth**
...
### المهمة 0.5: بناء RBAC

**الملفات:**
- إنشاء: `service/common/auth/Rbac.php`
- إنشاء: `service/common/auth/middleware/RbacMiddleware.php`
...
### المهمة 0.6: بناء بنية i18n

**الملفات:**
- إنشاء: `service/common/i18n/I18n.php`
- إنشاء: `service/common/i18n/middleware/LocaleMiddleware.php`
- إنشاء: `service/i18n/zh-CN/messages.php`
- إنشاء: `service/i18n/en-US/messages.php`
...
- [ ] **الخطوة 2: إنشاء LocaleMiddleware**
...
- [ ] **الخطوة 3: إنشاء ملفات رسائل i18n**
...
### المهمة 0.7: بناء وسائط الأمان

**الملفات:**
- إنشاء: `service/common/security/WafMiddleware.php`
- إنشاء: `service/common/security/RateLimitMiddleware.php`
- إنشاء: `service/common/security/CorsMiddleware.php`
- إنشاء: `service/common/security/AuditLogger.php`
- إنشاء: `service/common/security/LogSanitizer.php`

- [ ] **الخطوة 1: إنشاء WafMiddleware**
...
- [ ] **الخطوة 2: إنشاء RateLimitMiddleware**
...
### المهمة 0.8: إنشاء نقطة نهاية الصحة وتسجيل خط أنابيب الوسائط

**الملفات:**
- إنشاء: `service/app/controller/HealthController.php`
- تعديل: `service/start.php` (تسجيل الوسائط)
- تعديل: `service/config/route.php` (إن وجد، وإلا إنشاؤه)
...
- [ ] **الخطوة 2: تسجيل الوسائط العامة في start.php**
...
### المهمة 0.9: التحقق من اكتمال المرحلة 0

- [ ] **الخطوة 1: تشغيل خادم تطوير webman**

التشغيل: `cd service && php start.php start`
المتوقع: يبدأ الخادم على http://0.0.0.0:8787

- [ ] **الخطوة 2: اختبار نقطة نهاية الصحة**

التشغيل: `curl http://localhost:8787/health`
المتوقع: `{"code":0,"message":"ok","data":{"status":"healthy",...}}`

- [ ] **الخطوة 3: اختبار حد المعدل**

التشغيل: `for i in {1..10}; do curl -s http://localhost:8787/health; done`
المتوقع: كلها تعيد 200 (الصحة ليس لها حد معدل صارم)

- [ ] **الخطوة 4: اختبار WAF**

التشغيل: `curl -X POST http://localhost:8787/api/auth/login -d "email=test' OR '1'='1"`
المتوقع: `{"code":403,"message":"Request blocked by WAF"}`

- [ ] **الخطوة 5: التحقق من اتصال قاعدة البيانات**

التشغيل: `php service/database/migrate.php`
المتوقع: تعمل جميع الترحيلات دون أخطاء.

- [ ] **الخطوة 6: الالتزام بملف القفل**
...
**اكتملت المرحلة 0.** الأساس جاهز: سقالات المشروع، أكثر من 50 جدولًا في قاعدة البيانات، مصادقة JWT، RBAC، i18n، WAF + حد المعدل + CORS، سجلات التدقيق.
