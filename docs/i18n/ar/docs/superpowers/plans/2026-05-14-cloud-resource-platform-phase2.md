# المرحلة 2: تسليم الموارد — خطة التنفيذ
...
**الهدف:** تحقيق الفتح التلقائي للموارد بعد الدفع، وإدارة النطاقات، وانضمام الموردين وإدارتهم. تتحكم Proxmox API في إنشاء/إدارة الأجهزة الافتراضية VM وعناوين IP والأقراص على الأجهزة الفعلية.

**البنية:** معمارية إضافات Provider (واجهة ResourceProvider)، عامل Worker غير متزامن يستقصي provision_tasks، معمارية قائمة على الأحداث (OrderPaid → ProvisioningService → ProviderFactory → ProxmoxProvider/AwsProvider/etc).

**حزمة التقنيات:** PHP 8.2+، webman redis-queue، Proxmox VE REST API، Guzzle HTTP client

---

### المهمة 2.1: نواة محرك التوفير

**الملفات:**
- إنشاء: `service/app/provisioning/service/ProvisioningService.php`
- إنشاء: `service/app/provisioning/service/ProviderFactory.php`
- إنشاء: `service/app/provisioning/service/ProviderInterface.php`
- إنشاء: `service/app/provisioning/model/Resource.php`
- إنشاء: `service/app/provisioning/model/ProvisionTask.php`
- إنشاء: `service/app/provisioning/listener/OrderPaidListener.php`
- إنشاء: `service/app/provisioning/queue/ProvisionWorker.php`

- [ ] **الخطوة 1: إنشاء ProviderInterface**
...
- [ ] **الخطوة 2: إنشاء ProvisionResult و ResourceStatus**
...
- [ ] **الخطوة 3: إنشاء ProviderFactory**
...
- [ ] **الخطوة 4: إنشاء ProvisioningService**
...
- [ ] **الخطوة 5: إنشاء OrderPaidListener**
...
- [ ] **الخطوة 6: إنشاء ProvisionWorker (مهمة قائمة انتظار)**
...
### المهمة 2.2: تنفيذ موفر Proxmox

**الملفات:**
- إنشاء: `service/app/provisioning/provider/ProxmoxApi.php`
- إنشاء: `service/app/provisioning/provider/ProxmoxProvider.php`
- إنشاء: `service/app/provisioning/provider/HostSelector.php`
- إنشاء: `service/app/provisioning/model/HostMachine.php`
- إنشاء: `service/app/provisioning/model/IpPool.php`

- [ ] **الخطوة 1: إنشاء عميل HTTP ProxmoxApi**
...
- [ ] **الخطوة 3: إنشاء ProxmoxProvider**
...
### المهمة 2.3: خدمة النطاقات

**الملفات:**
- إنشاء: `service/app/domain/controller/DomainController.php`
- إنشاء: `service/app/domain/service/DomainService.php`
- إنشاء: `service/app/domain/model/DomainTld.php`
- إنشاء: `service/app/domain/model/DnsZone.php`
- إنشاء: `service/app/domain/model/DnsRecord.php`

- [ ] **الخطوة 1: إنشاء DomainService (الطرق الأساسية)**
...
### المهمة 2.4: نظام الموردين

**الملفات:**
- إنشاء: `service/app/supplier/controller/SupplierController.php`
- إنشاء: `service/app/supplier/service/SupplierService.php`
- إنشاء: `service/app/supplier/model/Supplier.php`
- إنشاء: `service/app/supplier/model/SupplierSettlement.php`

- [ ] **الخطوة 1: إنشاء SupplierService (منطق الأعمال الأساسي)**
...
### المهمة 2.5: ربط مسارات المرحلة 2 وتسجيل قائمة الانتظار

- [ ] **الخطوة 1: تحديث إعداد المسارات**
...
- [ ] **الخطوة 2: تسجيل ProvisionWorker في إعداد قائمة الانتظار**
...
**اكتملت المرحلة 2.** يؤدي الدفع الآن إلى الفتح التلقائي للموارد عبر Proxmox (أو واجهات برمجة تطبيقات السحابة الخارجية). يمكن للمستخدمين إدارة مواردهم ونطاقاتهم و DNS. يمكن للموردين الانضمام وإدراج المنتجات واستلام التسويات.
