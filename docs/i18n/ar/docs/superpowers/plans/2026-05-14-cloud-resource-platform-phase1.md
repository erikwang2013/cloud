# المرحلة 1: سلسلة المعاملات الأساسية — خطة التنفيذ
...
**الهدف:** تحقيق حلقة المعاملات الكاملة من تسجيل المستخدم وتسجيل الدخول حتى اكتمال الدفع. يمكن للمستخدم التسجيل وتصفح المنتجات وتقديم الطلبات واختيار طريقة الدفع وإتمام الدفع.

**البنية:** كل وحدة في دليل مستقل (controller/service/model)، معمارية تعتمد على الأحداث (حدث OrderPaid يطلق العمليات اللاحقة)، وقائمة انتظار Redis لمعالجة المهام غير المتزامنة.

**حزمة التقنيات:** PHP 8.2+، webman، Eloquent ORM، Redis، Stripe PHP SDK

---

### المهمة 1.1: تسجيل المستخدم وتسجيل الدخول

**الملفات:**
- إنشاء: `service/app/user/controller/AuthController.php`
- إنشاء: `service/app/user/service/AuthService.php`
- إنشاء: `service/app/user/model/User.php`
- إنشاء: `service/app/user/model/RefreshToken.php`
- إنشاء: `tests/User/AuthTest.php`

- [ ] **الخطوة 1: إنشاء نموذج User**
...
### المهمة 1.2: كتالوج المنتجات (القائمة، التفاصيل، البحث)

**الملفات:**
- إنشاء: `service/app/product/controller/ProductController.php`
- إنشاء: `service/app/product/service/ProductService.php`
- إنشاء: `service/app/product/model/Product.php`
- إنشاء: `service/app/product/model/ProductSku.php`
- إنشاء: `service/app/product/model/Region.php`

- [ ] **الخطوة 1: إنشاء نموذج Product مع العلاقات**
...
- [ ] **الخطوة 3: إنشاء ProductController**
...
### المهمة 1.3: سلة التسوق وإنشاء الطلب

**الملفات:**
- إنشاء: `service/app/order/controller/CartController.php`
- إنشاء: `service/app/order/controller/OrderController.php`
- إنشاء: `service/app/order/service/CartService.php`
- إنشاء: `service/app/order/service/OrderService.php`
- إنشاء: `service/app/order/model/Order.php`
- إنشاء: `service/app/order/model/OrderItem.php`

- [ ] **الخطوة 1: إنشاء نموذج Order وخدمة الطلبات (المنطق الأساسي)**
...
- [ ] **الخطوة 2: إنشاء OrderController**
...
### المهمة 1.4: تكامل الدفع (Stripe + الموجّه)

**الملفات:**
- إنشاء: `service/app/payment/controller/PaymentController.php`
- إنشاء: `service/app/payment/service/PaymentService.php`
- إنشاء: `service/app/payment/service/PaymentRouter.php`
- إنشاء: `service/app/payment/service/Channels/StripeChannel.php`
- إنشاء: `service/app/payment/service/Channels/CryptoChannel.php`
- إنشاء: `service/app/payment/model/PaymentChannel.php`
- إنشاء: `service/app/payment/model/PaymentTransaction.php`
- إنشاء: `service/app/payment/event/OrderPaid.php`

- [ ] **الخطوة 1: إنشاء PaymentRouter**
...
- [ ] **الخطوة 3: إنشاء حدث OrderPaid و PaymentController**
...
### المهمة 1.5: ربط المسارات والاختبار الشامل

- [ ] **الخطوة 1: تحديث إعداد المسارات** — `service/config/route.php`
...
- [ ] **الخطوة 2: سكربت الاختبار الشامل اليدوي**
...
**اكتملت المرحلة 1.** يمكن للمستخدمين الآن: التسجيل ← تسجيل الدخول ← تصفح المنتجات ← إضافة إلى السلة ← إنشاء الطلب ← عرض طرق الدفع ← الدفع عبر Stripe. يتم إطلاق حدث OrderPaid وهو جاهز لتكامل توفير الموارد في المرحلة 2.
