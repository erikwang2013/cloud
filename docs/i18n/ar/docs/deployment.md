...
- [ ] `GET /api/status` ← 200
- [ ] `GET /api/products` ← 200 (JSON صالح)
- [ ] `POST /api/auth/login` (بلا body) ← 422 (التحقق من المعاملات)
- [ ] `GET /api/user/profile` (بلا token) ← 401 (المصادقة)
- [ ] رأس الإصدار: `curl -H 'X-Api-Version: v99' /api/products` ← 400
