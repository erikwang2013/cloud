...
<p align="center">
  <p align="center">
  <img src="docs/diagrams/c.svg" alt="يونتون، تميمة مشروع CloudPlatform" width="260">
</p>

<p align="center">**يونتون** — تميمة المشروع</p>
</p>
...
- **بوابة المستخدم**: افتح عنوان خدمة API (الافتراضي `http://localhost:8787`)، سجّل حسابًا ثم سجّل الدخول. يدعم OAuth من Google / Apple والتحقق بخطوتين TOTP
- **لوحة الإدارة**: افتح `http://localhost:8787/app/admin` في المتصفح (اللوحة مثيل مستقل، المنفذ 8788) وسجّل الدخول بحساب المسؤول الذي أنشأه معالج التثبيت

### الوظائف الشائعة للوحة

- **لوحة المعلومات**: إحصائيات طلبات اليوم / الإيرادات / المستخدمين الجدد / الموارد النشطة، مخطط اتجاه الإيرادات لـ 30 يومًا، تصدير PDF
- **مركز التقارير**: تقارير الطلبات، ترتيب المنتجات، إحصائيات القنوات، نمو المستخدمين، تصدير Excel
- **الإدارة اليومية**: المستخدمون / المنتجات / الطلبات / الموردون / التذاكر / النطاقات / CDN، مراجعة KYC، استرداد الأموال، الموافقة على التسويات والسحوبات
- **إعدادات النظام**: قنوات الدفع، حسابات CDN، Webhook، قوالب الإشعارات، مقالات المساعدة، سجلات التدقيق

### بناء التطبيقات

- **عميل Flutter** (`apps/flutter/`): iOS / Android / Web / Linux / macOS / Windows. `flutter pub get` للتبعيات، `flutter run` للتصحيح، `flutter build apk` / `flutter build ios` / `flutter build web` للتغليف
- **عميل HarmonyOS** (`apps/harmonyos/`): تطبيق ArkTS أصلي — افتح مشروع `entry` في DevEco Studio للبناء والتشغيل
