...
echo ""
echo "=== API version path ==="
# Versioned path works (version comes from the URL path, no version header needed)
code=$(curl -s -o /dev/null -w '%{http_code}' "$BASE/api/v1/products" -H 'Content-Type: application/json')
[ "$code" = "200" ] && echo "  PASS versioned path /api/v1/products → $code" && ((PASS++)) || echo "  FAIL versioned path /api/v1/products → $code" && ((FAIL++))

# Unsupported version in URL path
code=$(curl -s -o /dev/null -w '%{http_code}' "$BASE/api/v99/products" -H 'Content-Type: application/json')
[ "$code" = "400" ] && echo "  PASS unsupported version v99 → $code" && ((PASS++)) || echo "  FAIL unsupported version v99 → $code" && ((FAIL++))
