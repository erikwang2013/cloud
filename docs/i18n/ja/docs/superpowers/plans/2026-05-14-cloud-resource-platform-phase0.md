...
> **エージェントワーカー向け:** 必須サブスキル: この計画をタスク単位で実装するには superpowers:subagent-driven-development（推奨）または superpowers:executing-plans を使用してください。手順はチェックボックス（`- [ ]`）構文で進捗を追跡します。

**ゴール:** プロジェクトのスキャフォールド、データベースマイグレーション、共通ライブラリ、認証認可、多言語インフラ、セキュリティミドルウェアを構築し、以降の全フェーズの基盤を提供します。

**アーキテクチャ:** webman モジュール型モノリス。ビジネスドメインごとに独立したディレクトリモジュールに分割し、共通インフラは common/ 配下に配置します。データベースは MySQL + Eloquent ORM、キャッシュ/キューは Redis を使用します。

**技術スタック:** PHP 8.2+, webman 1.6+, illuminate/database, firebase/php-jwt, redis, monolog
...
### タスク 0.1: webman プロジェクトと依存関係の初期化

**対象ファイル:**
- 新規作成: `service/composer.json`

- [ ] **ステップ 1: composer.json の作成**
...
実行: `cd service && composer install`
期待結果: 全パッケージがインストールされ、vendor/ が作成される。

- [ ] **ステップ 3: config/app.php の作成**
...
- [ ] **ステップ 4: config/database.php の作成**
...
- [ ] **ステップ 7: config/security.php の作成**
...
### タスク 0.2: 全データベースマイグレーションの作成

**対象ファイル:**
- 新規作成: `service/database/migrations/0001_create_users_tables.php`

- [ ] **ステップ 1: users マイグレーションの作成**
...
- [ ] **ステップ 2: products マイグレーションの作成** — `service/database/migrations/0002_create_product_tables.php`
...
- [ ] **ステップ 3: orders マイグレーションの作成** — `service/database/migrations/0003_create_order_tables.php`
...
- [ ] **ステップ 4: マイグレーション 0004〜0010**

同じパターンで残りのマイグレーションを続行します:
...
それぞれ上記と同じパターンで `Capsule::schema()->create(...)` を使用します。

- [ ] **ステップ 5: マイグレーションランナーの作成** — `service/database/migrate.php`
...
実行: `php service/database/migrate.php`
期待結果: 全テーブルが正常に作成される。
...
### タスク 0.3: 共通ヘルパーライブラリの構築

**対象ファイル:**
- 新規作成: `service/common/helper/Response.php`
- 新規作成: `service/common/helper/Validator.php`

- [ ] **ステップ 1: Response ヘルパーの作成**
...
**対象ファイル:**
- 新規作成: `service/common/auth/JwtAuth.php`
- 新規作成: `service/common/auth/middleware/AuthMiddleware.php`
...
**対象ファイル:**
- 新規作成: `service/common/auth/Rbac.php`
- 新規作成: `service/common/auth/middleware/RbacMiddleware.php`
...
### タスク 0.6: i18n インフラの構築

**対象ファイル:**
- 新規作成: `service/common/i18n/I18n.php`
- 新規作成: `service/common/i18n/middleware/LocaleMiddleware.php`
- 新規作成: `service/i18n/zh-CN/messages.php`
- 新規作成: `service/i18n/en-US/messages.php`
...
### タスク 0.7: セキュリティミドルウェアの構築

**対象ファイル:**
- 新規作成: `service/common/security/WafMiddleware.php`
- 新規作成: `service/common/security/RateLimitMiddleware.php`
- 新規作成: `service/common/security/CorsMiddleware.php`
- 新規作成: `service/common/security/AuditLogger.php`
- 新規作成: `service/common/security/LogSanitizer.php`

- [ ] **ステップ 1: WafMiddleware の作成**
...
- [ ] **ステップ 2: RateLimitMiddleware の作成**
...
### タスク 0.8: ヘルスエンドポイントの作成とミドルウェアパイプラインの登録

**対象ファイル:**
- 新規作成: `service/app/controller/HealthController.php`
- 変更: `service/start.php`（ミドルウェア登録）
- 変更: `service/config/route.php`（存在する場合。なければ作成）

- [ ] **ステップ 1: HealthController の作成**
...
- [ ] **ステップ 2: start.php へのグローバルミドルウェアの登録**
...
### タスク 0.9: Phase 0 の完了確認

- [ ] **ステップ 1: webman 開発サーバーの起動**

実行: `cd service && php start.php start`
期待結果: http://0.0.0.0:8787 でサーバーが起動する。

- [ ] **ステップ 2: ヘルスエンドポイントのテスト**

実行: `curl http://localhost:8787/health`
期待結果: `{"code":0,"message":"ok","data":{"status":"healthy",...}}`

- [ ] **ステップ 3: レート制限のテスト**

実行: `for i in {1..10}; do curl -s http://localhost:8787/health; done`
期待結果: すべて 200 が返る（health には厳格なレート制限がない）。

- [ ] **ステップ 4: WAF のテスト**

実行: `curl -X POST http://localhost:8787/api/auth/login -d "email=test' OR '1'='1"` 
期待結果: `{"code":403,"message":"Request blocked by WAF"}`

- [ ] **ステップ 5: DB 接続の確認**

実行: `php service/database/migrate.php`
期待結果: 全マイグレーションがエラーなく実行される。
...
**Phase 0 完了。** 基盤準備完了: プロジェクトスキャフォールド、50+ の DB テーブル、JWT 認証、RBAC、i18n、WAF + レート制限 + CORS、監査ログ。
