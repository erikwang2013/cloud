...
**ゴール:** ユーザー登録・ログインから支払い完了までの完全な取引クローズドループを実装します。ユーザーは登録、商品閲覧、注文、支払い方法の選択、支払い完了まで行えます。

**アーキテクチャ:** 各モジュールは独立したディレクトリ（controller/service/model）を持ち、イベント駆動（OrderPaid イベントが後続フローをトリガー）で、Redis キューが非同期タスクを処理します。

**技術スタック:** PHP 8.2+, webman, Eloquent ORM, Redis, Stripe PHP SDK
...
**対象ファイル:**
- 新規作成: `service/app/user/controller/AuthController.php`
- 新規作成: `service/app/user/service/AuthService.php`
- 新規作成: `service/app/user/model/User.php`
- 新規作成: `service/app/user/model/RefreshToken.php`
- 新規作成: `tests/User/AuthTest.php`
...
### タスク 1.2: 商品カタログ（一覧、詳細、検索）

**対象ファイル:**
- 新規作成: `service/app/product/controller/ProductController.php`
- 新規作成: `service/app/product/service/ProductService.php`
- 新規作成: `service/app/product/model/Product.php`
- 新規作成: `service/app/product/model/ProductSku.php`
- 新規作成: `service/app/product/model/Region.php`

- [ ] **ステップ 1: リレーション付き Product モデルの作成**
...
### タスク 1.3: ショッピングカートと注文作成

**対象ファイル:**
- 新規作成: `service/app/order/controller/CartController.php`
- 新規作成: `service/app/order/controller/OrderController.php`
- 新規作成: `service/app/order/service/CartService.php`
- 新規作成: `service/app/order/service/OrderService.php`
- 新規作成: `service/app/order/model/Order.php`
- 新規作成: `service/app/order/model/OrderItem.php`

- [ ] **ステップ 1: Order モデルと注文サービス（中核ロジック）の作成**
...
### タスク 1.4: 決済統合（Stripe + ルーター）

**対象ファイル:**
- 新規作成: `service/app/payment/controller/PaymentController.php`
- 新規作成: `service/app/payment/service/PaymentService.php`
- 新規作成: `service/app/payment/service/PaymentRouter.php`
- 新規作成: `service/app/payment/service/Channels/StripeChannel.php`
- 新規作成: `service/app/payment/service/Channels/CryptoChannel.php`
- 新規作成: `service/app/payment/model/PaymentChannel.php`
- 新規作成: `service/app/payment/model/PaymentTransaction.php`
- 新規作成: `service/app/payment/event/OrderPaid.php`

- [ ] **ステップ 1: PaymentRouter の作成**
...
- [ ] **ステップ 3: OrderPaid イベントと PaymentController の作成**
...
### タスク 1.5: ルートの配線とエンドツーエンドテスト

- [ ] **ステップ 1: ルート設定の更新** — `service/config/route.php`
...
**Phase 1 完了。** ユーザーはこれで: 登録 → ログイン → 商品閲覧 → カート追加 → 注文作成 → 支払い方法の確認 → Stripe で支払い、まで行えます。OrderPaid イベントが発火し、Phase 2 のプロビジョニング統合に備えます。
