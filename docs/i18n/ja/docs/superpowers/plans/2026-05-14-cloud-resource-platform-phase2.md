...
**ゴール:** 支払い後のリソース自動開通、ドメイン管理、サプライヤー入居と管理を実装します。Proxmox API が物理マシンでの VM・IP・ディスクの作成/管理を制御します。

**アーキテクチャ:** Provider プラグインアーキテクチャ（ResourceProvider interface）、非同期 Worker が provision_tasks をポーリング、イベント駆動（OrderPaid → ProvisioningService → ProviderFactory → ProxmoxProvider/AwsProvider/etc）。

**技術スタック:** PHP 8.2+, webman redis-queue, Proxmox VE REST API, Guzzle HTTP client

---

### タスク 2.1: プロビジョニングエンジンの中核

**対象ファイル:**
- 新規作成: `service/app/provisioning/service/ProvisioningService.php`
- 新規作成: `service/app/provisioning/service/ProviderFactory.php`
- 新規作成: `service/app/provisioning/service/ProviderInterface.php`
- 新規作成: `service/app/provisioning/model/Resource.php`
- 新規作成: `service/app/provisioning/model/ProvisionTask.php`
- 新規作成: `service/app/provisioning/listener/OrderPaidListener.php`
- 新規作成: `service/app/provisioning/queue/ProvisionWorker.php`

- [ ] **ステップ 1: ProviderInterface の作成**
...
- [ ] **ステップ 2: ProvisionResult と ResourceStatus の作成**
...
- [ ] **ステップ 4: ProvisioningService の作成**
...
- [ ] **ステップ 6: ProvisionWorker の作成（キュージョブ）**
...
### タスク 2.2: Proxmox Provider の実装

**対象ファイル:**
- 新規作成: `service/app/provisioning/provider/ProxmoxApi.php`
- 新規作成: `service/app/provisioning/provider/ProxmoxProvider.php`
- 新規作成: `service/app/provisioning/provider/HostSelector.php`
- 新規作成: `service/app/provisioning/model/HostMachine.php`
- 新規作成: `service/app/provisioning/model/IpPool.php`

- [ ] **ステップ 1: ProxmoxApi HTTP クライアントの作成**
...
**対象ファイル:**
- 新規作成: `service/app/domain/controller/DomainController.php`
- 新規作成: `service/app/domain/service/DomainService.php`
- 新規作成: `service/app/domain/model/DomainTld.php`
- 新規作成: `service/app/domain/model/DnsZone.php`
- 新規作成: `service/app/domain/model/DnsRecord.php`

- [ ] **ステップ 1: DomainService の作成（主要メソッド）**
...
**対象ファイル:**
- 新規作成: `service/app/supplier/controller/SupplierController.php`
- 新規作成: `service/app/supplier/service/SupplierService.php`
- 新規作成: `service/app/supplier/model/Supplier.php`
- 新規作成: `service/app/supplier/model/SupplierSettlement.php`

- [ ] **ステップ 1: SupplierService の作成（主要ビジネスロジック）**
...
- [ ] **ステップ 2: キュー設定への ProvisionWorker の登録**
...
**Phase 2 完了。** 支払いが Proxmox（またはサードパーティクラウド API）経由のリソース自動プロビジョニングをトリガーします。ユーザーはリソース、ドメイン、DNS を管理できます。サプライヤーは入居し、商品を出品し、決済を受け取れます。
