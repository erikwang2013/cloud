...
### API 엔드포인트 점검
- [ ] `GET /api/status` → 200
- [ ] `GET /api/products` → 200 (유효 JSON)
- [ ] `POST /api/auth/login` (body 없음) → 422 (파라미터 검증)
- [ ] `GET /api/user/profile` (token 없음) → 401 (인증)
- [ ] 버전 헤더: `curl -H 'X-Api-Version: v99' /api/products` → 400
