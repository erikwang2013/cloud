# Phase 1: 핵심 거래 체인 — 구현 계획

> **에이전트 워커용:** 필수 서브 스킬: superpowers:subagent-driven-development (권장) 또는 superpowers:executing-plans를 사용하여 이 계획을 태스크 단위로 구현하세요.

**목표:** 사용자 가입·로그인에서 결제 완료까지의 완전한 거래 폐루프를 구현합니다. 사용자는 가입하고, 상품을 둘러보고, 주문하고, 결제 수단을 선택해 결제를 완료할 수 있습니다.

**아키텍처:** 각 모듈은 독립 디렉터리 (controller/service/model)를 가지며, 이벤트 드리븐 (OrderPaid 이벤트가 후속 프로세스 트리거) 방식이고, Redis 큐가 비동기 태스크를 처리합니다.

**기술 스택:** PHP 8.2+, webman, Eloquent ORM, Redis, Stripe PHP SDK

---

### Task 1.1: 사용자 가입 및 로그인

**파일:**
- 생성: `service/app/user/controller/AuthController.php`
- 생성: `service/app/user/service/AuthService.php`
- 생성: `service/app/user/model/User.php`
- 생성: `service/app/user/model/RefreshToken.php`
- 생성: `tests/User/AuthTest.php`
...
### Task 1.2: 상품 카탈로그 (목록, 상세, 검색)

**파일:**
- 생성: `service/app/product/controller/ProductController.php`
- 생성: `service/app/product/service/ProductService.php`
- 생성: `service/app/product/model/Product.php`
- 생성: `service/app/product/model/ProductSku.php`
- 생성: `service/app/product/model/Region.php`

- [ ] **Step 1: 관계가 포함된 Product 모델 생성**
...
### Task 1.3: 장바구니 및 주문 생성

**파일:**
- 생성: `service/app/order/controller/CartController.php`
- 생성: `service/app/order/controller/OrderController.php`
- 생성: `service/app/order/service/CartService.php`
- 생성: `service/app/order/service/OrderService.php`
- 생성: `service/app/order/model/Order.php`
- 생성: `service/app/order/model/OrderItem.php`

- [ ] **Step 1: Order 모델 및 주문 서비스 생성 (핵심 로직)**
...
### Task 1.4: 결제 연동 (Stripe + Router)

**파일:**
- 생성: `service/app/payment/controller/PaymentController.php`
- 생성: `service/app/payment/service/PaymentService.php`
- 생성: `service/app/payment/service/PaymentRouter.php`
- 생성: `service/app/payment/service/Channels/StripeChannel.php`
- 생성: `service/app/payment/service/Channels/CryptoChannel.php`
- 생성: `service/app/payment/model/PaymentChannel.php`
- 생성: `service/app/payment/model/PaymentTransaction.php`
- 생성: `service/app/payment/event/OrderPaid.php`

- [ ] **Step 1: PaymentRouter 생성**
...
- [ ] **Step 3: OrderPaid 이벤트와 PaymentController 생성**
...
### Task 1.5: 라우트 연결 및 엔드투엔드 테스트

- [ ] **Step 1: 라우트 설정 업데이트** — `service/config/route.php`
...
**Phase 1 완료.** 이제 사용자는: 가입 → 로그인 → 상품 둘러보기 → 장바구니 담기 → 주문 생성 → 결제 수단 확인 → Stripe로 결제를 할 수 있습니다. OrderPaid 이벤트가 발생하며 Phase 2 프로비저닝 연동 준비가 완료됩니다.
