# Phase 0: 기반 인프라 — 구현 계획

> **에이전트 워커용:** 필수 서브 스킬: superpowers:subagent-driven-development (권장) 또는 superpowers:executing-plans를 사용하여 이 계획을 태스크 단위로 구현하세요. 단계는 체크박스 (`- [ ]`) 구문으로 추적합니다.

**목표:** 프로젝트 스캐폴딩, 데이터베이스 마이그레이션, 공용 라이브러리, 인증/권한, 다중 언어 인프라, 보안 미들웨어를 구축하여 이후 모든 단계의 기반을 제공합니다.

**아키텍처:** webman 모듈형 모노리스, 비즈니스 도메인별 독립 디렉터리 모듈로 분리하며 공용 인프라는 common/ 아래에 둡니다. 데이터베이스는 MySQL + Eloquent ORM, 캐시/큐는 Redis를 사용합니다.

**기술 스택:** PHP 8.2+, webman 1.6+, illuminate/database, firebase/php-jwt, redis, monolog
...
### Task 0.1: webman 프로젝트 및 의존성 초기화
...
기대 결과: 모든 패키지 설치됨, vendor/ 생성됨.

- [ ] **Step 3: config/app.php 생성**
...
### Task 0.2: 전체 데이터베이스 마이그레이션 생성

**파일:**
- 생성: `service/database/migrations/0001_create_users_tables.php`

- [ ] **Step 1: users 마이그레이션 생성**
...
- [ ] **Step 2: products 마이그레이션 생성** — `service/database/migrations/0002_create_product_tables.php`
...
- [ ] **Step 3: orders 마이그레이션 생성** — `service/database/migrations/0003_create_order_tables.php`
...
- [ ] **Step 4: 마이그레이션 0004-0010**

동일한 패턴으로 나머지 마이그레이션을 계속 진행합니다:
...
각각 위와 동일한 패턴으로 `Capsule::schema()->create(...)`를 사용합니다.

- [ ] **Step 5: 마이그레이션 러너 생성** — `service/database/migrate.php`
...
실행: `php service/database/migrate.php`
기대 결과: 모든 테이블이 성공적으로 생성됨.
...
### Task 0.3: 공용 헬퍼 라이브러리 구축

**파일:**
- 생성: `service/common/helper/Response.php`
- 생성: `service/common/helper/Validator.php`

- [ ] **Step 1: Response 헬퍼 생성**
...
**파일:**
- 생성: `service/common/auth/JwtAuth.php`
- 생성: `service/common/auth/middleware/AuthMiddleware.php`
...
**파일:**
- 생성: `service/common/auth/Rbac.php`
- 생성: `service/common/auth/middleware/RbacMiddleware.php`
...
### Task 0.6: i18n 인프라 구축

**파일:**
- 생성: `service/common/i18n/I18n.php`
- 생성: `service/common/i18n/middleware/LocaleMiddleware.php`
- 생성: `service/i18n/zh-CN/messages.php`
- 생성: `service/i18n/en-US/messages.php`
...
**파일:**
- 생성: `service/common/security/WafMiddleware.php`
- 생성: `service/common/security/RateLimitMiddleware.php`
- 생성: `service/common/security/CorsMiddleware.php`
- 생성: `service/common/security/AuditLogger.php`
- 생성: `service/common/security/LogSanitizer.php`

- [ ] **Step 1: WafMiddleware 생성**
...
### Task 0.8: 헬스 엔드포인트 생성 및 미들웨어 파이프라인 등록

**파일:**
- 생성: `service/app/controller/HealthController.php`
- 수정: `service/start.php` (미들웨어 등록)
- 수정: `service/config/route.php` (존재하지 않으면 생성)
...
### Task 0.9: Phase 0 완성도 검증

- [ ] **Step 1: webman dev 서버 시작**

실행: `cd service && php start.php start`
기대 결과: 서버가 http://0.0.0.0:8787에서 시작됨

- [ ] **Step 2: 헬스 엔드포인트 테스트**

실행: `curl http://localhost:8787/health`
기대 결과: `{"code":0,"message":"ok","data":{"status":"healthy",...}}`

- [ ] **Step 3: 속도 제한 테스트**

실행: `for i in {1..10}; do curl -s http://localhost:8787/health; done`
기대 결과: 전부 200 반환 (health는 엄격한 속도 제한 없음)

- [ ] **Step 4: WAF 테스트**

실행: `curl -X POST http://localhost:8787/api/auth/login -d "email=test' OR '1'='1"` 
기대 결과: `{"code":403,"message":"Request blocked by WAF"}`

- [ ] **Step 5: DB 연결 확인**

실행: `php service/database/migrate.php`
기대 결과: 모든 마이그레이션이 오류 없이 실행됨.
...
**Phase 0 완료.** 기반 준비 완료: 프로젝트 스캐폴딩, 50+ DB 테이블, JWT 인증, RBAC, i18n, WAF + 속도 제한 + CORS, 감사 로깅.
