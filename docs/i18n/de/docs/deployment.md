...
- [ ] `GET /api/status` → 200
- [ ] `GET /api/products` → 200 (gültiges JSON)
- [ ] `POST /api/auth/login` (ohne body) → 422 (Parametervalidierung)
- [ ] `GET /api/user/profile` (ohne token) → 401 (Authentifizierung)
- [ ] Versions-Header: `curl -H 'X-Api-Version: v99' /api/products` → 400
