# Phase 3: Betriebsoptimierung — Implementierungsplan
...
**Ziel:** Ticketsystem, Nachrichtenbenachrichtigungen, Monitoring-Alarme, Admin-Backend-Seiten, Client-App-Gerüst, Bereitstellungskonfiguration. Die Plattform erreicht den Status des regulären Betriebs.

**Architektur:** Tickets/Benachrichtigungen werden ereignisgesteuert + asynchron über die Redis-Queue verteilt. Das Admin-Backend basiert auf webman-admin. Die Client-Apps teilen sich die API-Schicht.

**Tech-Stack:** PHP 8.2+, webman-admin, webman redis-queue, Flutter 3.x, HarmonyOS ArkTS, Docker

---

### Aufgabe 3.1: Ticketsystem

**Dateien:**
- Erstellen: `service/app/ticket/controller/TicketController.php`
- Erstellen: `service/app/ticket/service/TicketService.php`
- Erstellen: `service/app/ticket/model/Ticket.php`
- Erstellen: `service/app/ticket/model/TicketMessage.php`
- Erstellen: `service/app/ticket/event/TicketCreated.php`
- Erstellen: `service/app/ticket/listener/AutoAssignListener.php`

- [ ] **Schritt 1: TicketService erstellen**
...
class TicketService
{
    // SLA-Fristen nach Priorität
    private array $slaMinutes = [
        'urgent'  => 30,
        'high'    => 120,
        'normal'  => 480,   // 8 Stunden
        'low'     => 1440,  // 24 Stunden
    ];
    ...
    public function create(int $userId, array $data): Ticket
    {
        ...
        // Ereignis für automatische Zuweisung auslösen
    ...
    public function reply(int $ticketId, int $senderId, string $senderType, string $content): TicketMessage
    {
        ...
        // Ticket wieder öffnen, wenn der Benutzer auf ein angehaltenes Ticket antwortet
        if ($ticket->status === 'on_hold' && $senderType === 'user') {
            $ticket->update(['status' => 'open']);
        }

        // Bei erster Antwort des Personals als in Bearbeitung markieren
    ...
    // Automatische Zuweisung: Round-Robin unter den Support-Mitarbeitern
    public function autoAssign(Ticket $ticket): void
    {
        ...
        // Einfaches Round-Robin: dem Mitarbeiter mit den wenigsten offenen Tickets zuweisen
...
- [ ] **Schritt 2: TicketController erstellen**
...
class TicketController
{
    ...
    // Admin: alle Tickets auflisten (mit Filtern)
...
- [ ] **Schritt 3: AutoAssignListener erstellen**
...
### Aufgabe 3.2: Benachrichtigungssystem

**Dateien:**
- Erstellen: `service/app/notification/service/NotificationDispatcher.php`
- Erstellen: `service/app/notification/queue/EmailSender.php`
- Erstellen: `service/app/notification/queue/SmsSender.php`
- Erstellen: `service/app/notification/queue/PushSender.php`
- Erstellen: `service/app/notification/model/Notification.php`
- Erstellen: `service/app/notification/model/NotificationTemplate.php`

- [ ] **Schritt 1: NotificationDispatcher erstellen**
...
{
    /**
     * Benachrichtigung an die bevorzugten Kanäle des Benutzers verteilen
     *
     * @param int    $userId   ID des Empfängers
     * @param string $code     Vorlagencode, z. B. 'order_paid', 'resource_expiring'
     * @param array  $data     Vorlagenvariablen: ['order_no' => '...', 'amount' => '...']
     * @param array  $channels Kanalüberschreibung, leer = Benutzereinstellung
     */
    public function dispatch(int $userId, string $code, array $data = [], array $channels = []): void
    {
        ...
        // Spracheinstellung des Benutzers abrufen
        ...
        // In die notifications-Tabelle schreiben (In-App)
        ...
        // Asynchrone Kanäle in die Queue stellen
...
- [ ] **Schritt 2: EmailSender-Queue-Worker erstellen**
...
class EmailSender implements Consumer
{
    ...
    {
        try {
            // Über den konfigurierten Mailer senden
            ...
            throw $e; // Erneut werfen für Queue-Wiederholung
        }
    }
}
```

- [ ] **Schritt 3: SmsSender-Queue-Worker erstellen**
...
class SmsSender implements Consumer
{
    ...
    {
        try {
            // Twilio SMS (oder Alibaba Cloud SMS für chinesische Benutzer)
...
- [ ] **Schritt 4: PushSender-Queue-Worker erstellen**
...
class PushSender implements Consumer
{
    ...
    {
        try {
            // Firebase Cloud Messaging (FCM) für Flutter
            $fcm = new \Google\Client();
            // ... FCM-Sendelogik mit Service-Account-Zugangsdaten
...
### Aufgabe 3.3: Monitoring und Alarme

**Dateien:**
- Erstellen: `service/app/monitor/controller/MonitorController.php`
- Erstellen: `service/app/monitor/service/ResourceMonitor.php`
- Erstellen: `service/app/monitor/service/AlertEngine.php`
- Erstellen: `service/app/monitor/cron/CollectMetrics.php`
- Erstellen: `service/app/monitor/cron/CheckExpirations.php`

- [ ] **Schritt 1: ResourceMonitor erstellen**
...
class ResourceMonitor
{
    ...
    // Metriken aller aktiven Ressourcen sammeln
    public function collectAllMetrics(): void
    {
        ...
        foreach ($resources as $resource) {
            ...
            // Prüfen, ob die VM ausgefallen ist
    ...
    private function checkDowntime(Resource $resource): void
    {
        ...
        Redis::expire($key, 600); // 10-Minuten-Fenster
    ...
    // Ressourcen prüfen, die dem Ablauf nahe sind
    public function checkExpirations(): void
    {
        $windows = [7, 3, 1]; // Tage vor Ablauf
...
- [ ] **Schritt 2: AlertEngine erstellen**
...
class AlertEngine
{
    ...
    public function trigger(string $ruleCode, $resource, array $context = []): void
    {
        ...
        // P0/P1: zusätzlich Bereitschaftspersonal benachrichtigen
...
- [ ] **Schritt 3: Cron-Aufgaben erstellen**

```php
<?php
// service/app/monitor/cron/CollectMetrics.php
// Alle 5 Minuten über crontab ausführen:
...
// Stündlich über crontab ausführen:
...
- [ ] **Schritt 4: Admin-Monitoring-Dashboard-Controller erstellen**
...
### Aufgabe 3.4: Admin-Backend (webman-admin-Seiten)

**Dateien:**
- Erstellen: `service/app/admin/controller/DashboardController.php`
- Erstellen: `service/app/admin/controller/UserController.php`
- Erstellen: `service/app/admin/controller/ProductController.php`
- Erstellen: `service/app/admin/controller/OrderController.php`
- Erstellen: `service/app/admin/controller/PaymentController.php`
- Erstellen: `service/app/admin/controller/SupplierController.php`

- [ ] **Schritt 1: DashboardController erstellen**
...
- [ ] **Schritt 2: Admin-Controller für die Bestellverwaltung erstellen**
...
class OrderController
{
    ...
    public function refund($request, int $id)
    {
        ...
        // Erstattungsdatensatz erstellen
        ...
        // Zur Genehmigung einreichen (2-Schritte-Prozess)
        // In Produktion läuft dies über einen Genehmigungsworkflow
...
### Aufgabe 3.5: Flutter-App-Gerüst

**Dateien:**
- Erstellen: Projektstruktur `apps/flutter/`
- Erstellen: `apps/flutter/lib/core/` (network, auth, i18n, theme)
- Erstellen: `apps/flutter/lib/features/` (auth, products, orders, resources)

- [ ] **Schritt 1: Flutter-Projekt initialisieren**

```bash
cd apps && flutter create --org com.cloudplatform --project-name cloud_platform flutter
```

- [ ] **Schritt 2: Kern-Netzwerkschicht erstellen** — `apps/flutter/lib/core/network/api_client.dart`
...
- [ ] **Schritt 3: Feature-Struktur erstellen**
...
### Aufgabe 3.6: HarmonyOS-App-Gerüst

**Dateien:**
- Erstellen: Projektstruktur `apps/harmonyos/`

- [ ] **Schritt 1: HarmonyOS-Projektstruktur erstellen**
...
- [ ] **Schritt 2: ApiClient erstellen (ArkTS)**
...
### Aufgabe 3.7: Docker- und Bereitstellungskonfiguration

**Dateien:**
- Erstellen: `docker/Dockerfile`
- Erstellen: `docker/docker-compose.yml`
- Erstellen: `docker/nginx.conf`
- Erstellen: `docker/supervisor.conf`
...
- [ ] **Schritt 3: nginx.conf erstellen**
...
### Aufgabe 3.8: Berichtsdienst

**Dateien:**
- Erstellen: `service/app/report/service/ReportService.php`
- Erstellen: `service/app/report/controller/ReportController.php`

- [ ] **Schritt 1: ReportService erstellen**
...
### Aufgabe 3.9: Abschließende Integration — alle Routen, Queues und Ereignisse verdrahten

- [ ] **Schritt 1: Routenkonfiguration vervollständigen** — `service/config/route.php`

```php
<?php
// Endgültige Routenkonfiguration, die alle Module kombiniert
...
// Produkte (öffentliche Leseoperationen)
...
- [ ] **Schritt 2: Queue-Consumer in der Konfiguration registrieren**
...
- [ ] **Schritt 3: Ereignis-Listener in start.php registrieren**
...
- [ ] **Schritt 4: Crontab konfigurieren**
...
- [ ] **Schritt 5: Abschließender Commit**
...
**Phase 3 abgeschlossen. Die Plattform ist voll betriebsbereit:**
- Benutzer: registrieren → durchstöbern → bestellen → bezahlen → automatische Bereitstellung → Ressourcen verwalten → Tickets einreichen
- Admin: Dashboard → Benutzer/Produkte/Bestellungen/Zahlungen verwalten → KYC prüfen → Tickets bearbeiten → Berichte ansehen
- Supplier: bewerben → genehmigt werden → Produkte einstellen → Abrechnungen erhalten → auszahlen
- Monitoring: Metriksammlung alle 5 Minuten, Alarme bei Ausfällen/Abläufen
- Bereitstellung: Docker-Compose-Start mit einem Befehl
