# Phase 2: Ressourcenbereitstellung — Implementierungsplan
...
**Ziel:** Automatische Ressourcenbereitstellung nach Zahlungseingang, Domänenverwaltung sowie Supplier-Onboarding und -Verwaltung implementieren. Die Proxmox-API steuert physische Maschinen zum Erstellen/Verwalten von VMs, IPs und Festplatten.

**Architektur:** Provider-Plugin-Architektur (ResourceProvider-Interface), asynchroner Worker pollt provision_tasks, ereignisgesteuert (OrderPaid → ProvisioningService → ProviderFactory → ProxmoxProvider/AwsProvider/etc).

**Tech-Stack:** PHP 8.2+, webman redis-queue, Proxmox VE REST API, Guzzle HTTP-Client

---

### Aufgabe 2.1: Kern der Bereitstellungs-Engine

**Dateien:**
- Erstellen: `service/app/provisioning/service/ProvisioningService.php`
- Erstellen: `service/app/provisioning/service/ProviderFactory.php`
- Erstellen: `service/app/provisioning/service/ProviderInterface.php`
- Erstellen: `service/app/provisioning/model/Resource.php`
- Erstellen: `service/app/provisioning/model/ProvisionTask.php`
- Erstellen: `service/app/provisioning/listener/OrderPaidListener.php`
- Erstellen: `service/app/provisioning/queue/ProvisionWorker.php`

- [ ] **Schritt 1: ProviderInterface erstellen**
...
- [ ] **Schritt 2: ProvisionResult und ResourceStatus erstellen**
...
- [ ] **Schritt 3: ProviderFactory erstellen**
...
- [ ] **Schritt 4: ProvisioningService erstellen**
...
class ProvisioningService
{
    ...
    public function __construct()
    {
        ...
        // Provider registrieren (wird in Aufgabe 2.2 befüllt)
    }

    // Wird vom OrderPaid-Ereignis aufgerufen
    ...
    private function getProductType(OrderItem $item): string
    {
        ...
        // Kategorie auf Produkttyp abbilden
    ...
    {
        // Prüfen, ob das Artikelprodukt einen Supplier hat -> Drittanbieter
        // Andernfalls -> selbst betrieben
        if ($item->product->supplier_id) {
            return $item->product->supplier->provider_code ?? 'aliyun';
        }
        return 'proxmox'; // selbst betrieben
    }
}
```

- [ ] **Schritt 5: OrderPaidListener erstellen**
...
- [ ] **Schritt 6: ProvisionWorker erstellen (Queue-Job)**
...
class ProvisionWorker implements Consumer
{
        ...
        foreach ($tasks as $task) {
            try {
                ...
                if ($result->status === 'success') {
                    ...
                    $delays  = [1, 5, 15, 60, 360, 86400]; // Minuten
    ...
    {
        // Monitoring-Alarm auslösen und automatisch Ticket erstellen
...
### Aufgabe 2.2: Proxmox-Provider implementieren

**Dateien:**
- Erstellen: `service/app/provisioning/provider/ProxmoxApi.php`
- Erstellen: `service/app/provisioning/provider/ProxmoxProvider.php`
- Erstellen: `service/app/provisioning/provider/HostSelector.php`
- Erstellen: `service/app/provisioning/model/HostMachine.php`
- Erstellen: `service/app/provisioning/model/IpPool.php`

- [ ] **Schritt 1: ProxmoxApi-HTTP-Client erstellen**
...
class ProxmoxApi
{
    ...
    public function __construct(HostMachine $host)
    {
        ...
        $this->http    = new Client([
            'verify'  => false, // internes Netzwerk
            'timeout' => 30,
        ]);
    }
    ...
    // Nächste verfügbare VMID aus dem Proxmox-Cluster abrufen
...
- [ ] **Schritt 2: HostSelector erstellen**
...
- [ ] **Schritt 3: ProxmoxProvider erstellen**
...
class ProxmoxProvider implements ProviderInterface
{
    ...
    public function create(ProvisionTask $task): ProvisionResult
    {
        ...
        try {
            ...
            // 6. Zugewiesene Host-Ressourcen aktualisieren
            $this->incrementHostAllocation($host, $specs, $diskSize);

            // 7. Festplattendatensatz erstellen
            ...
            // Netzwerkfehler oder Proxmox-API-Fehler — wiederholbar
    ...
    public function createDisk(ProvisionTask $task): ProvisionResult
    {
        try {
            ...
            // Nächsten verfügbaren Festplatten-Slot finden
    ...
    public function createIp(ProvisionTask $task): ProvisionResult
    {
        try {
            ...
            // Sekundäre Netzwerkschnittstelle hinzufügen
    ...
    public function renew(Resource $resource, int $months): ProvisionResult
    {
        // Proxmox-VM benötigt für Verlängerung keinen API-Aufruf — nur expired_at verlängern
        $resource->update([
    ...
    private function pickIp(IpPool $pool): string
    {
        ...
        // Einfache sequenzielle Zuweisung aus dem Pool-Bereich
...
### Aufgabe 2.3: Domänen-Service

**Dateien:**
- Erstellen: `service/app/domain/controller/DomainController.php`
- Erstellen: `service/app/domain/service/DomainService.php`
- Erstellen: `service/app/domain/model/DomainTld.php`
- Erstellen: `service/app/domain/model/DnsZone.php`
- Erstellen: `service/app/domain/model/DnsRecord.php`

- [ ] **Schritt 1: DomainService erstellen (Kernmethoden)**
...
class DomainService
{
    ...
    public function register(int $userId, string $domainName, string $tld, array $options = []): void
    {
        ...
        // Registrierungsaufgabe über die Bereitstellungs-Engine erstellen
...
### Aufgabe 2.4: Supplier-System

**Dateien:**
- Erstellen: `service/app/supplier/controller/SupplierController.php`
- Erstellen: `service/app/supplier/service/SupplierService.php`
- Erstellen: `service/app/supplier/model/Supplier.php`
- Erstellen: `service/app/supplier/model/SupplierSettlement.php`

- [ ] **Schritt 1: SupplierService erstellen (Kern-Geschäftslogik)**
...
class SupplierService
{
    ...
    public function approve(int $supplierId, int $adminId): void
    {
        ...
        // Benutzerrolle auf Supplier anheben
        \App\User\Model\User::where('id', $supplier->user_id)->update(['role' => 'supplier']);
    }

    // Abrechnung berechnen und erstellen
    ...
    // Globaler Scope für die Datenisolation von Supplyern
...
### Aufgabe 2.5: Phase-2-Routen verdrahten und Queue registrieren

- [ ] **Schritt 1: Routenkonfiguration aktualisieren**

```php
// Bereitstellungsrouten (nur Admin)
...
- [ ] **Schritt 2: ProvisionWorker in der Queue-Konfiguration registrieren**
...
**Phase 2 abgeschlossen.** Die Zahlung löst jetzt die automatische Ressourcenbereitstellung über Proxmox (oder Drittanbieter-Cloud-APIs) aus. Benutzer können ihre Ressourcen, Domänen und DNS verwalten. Supplier können beitreten, Produkte einstellen und Abrechnungen erhalten.
