...
**版本控制:** API 版本位于 URL 路径中（如 `/api/v1/...`）。不支持的版本返回 `400`。
...
| `/api` (敏感) | 全局 + Encryption + Auth + Confirmation | `/api/v1/orders/{id}/pay` |
| `/api/v1/supplier/external` | Version + SupplierApiKey | 供应商外部 API |
...
管理端 CDN 路由挂 `RbacMiddleware('cdn.manage')`，套餐变更写入审计日志（`admin_cdn_update_plan`）。服务商账号凭据通过 `/admin/api/v1/providers` CRUD 维护（RbacMiddleware `provider.config`，`code` 约定 `cdn-cloudflare` / `cdn-cloudfront` / `cdn-aliyun` / `cdn-tencent`，凭据 Encryptable 加密入库）。
